...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 取得批发商品总数 */
    $sql = "SELECT COUNT(*) FROM " . $xishan->table('wholesale') . " AS w, " . $xishan->table('goods') . " AS g " . $where;
    $count = $db->getOne($sql);
...
elseif ($_REQUEST['act'] == 'price_list')
{
    ...
    $sql = "SELECT * FROM " . $xishan->table('wholesale') .
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        foreach ($price_list as $attr_price)
        {
            if ($attr_price['attr'])
            {
                $sql = "SELECT attr_value FROM " . $xishan->table('goods_attr') .
...
elseif ($_REQUEST['act'] == 'submit_order')
{
    ...
    foreach ($_SESSION['wholesale_goods'] as $goods)
    {
        ...
        if (!empty($goods['goods_attr_id']))
        {
            ...
            $sql = "SELECT product_id FROM " . $xishan->table('products') . " WHERE goods_attr = '$goods_attr' AND goods_id = '" . $goods['goods_id'] . "'";
...
function wholesale_list($size, $page, $where)
{
    ...
    $sql = "SELECT w.*, g.goods_thumb, g.goods_name as goods_name " .
            "FROM " . $GLOBALS['xishan']->table('wholesale') . " AS w, " .
                      $GLOBALS['xishan']->table('goods') . " AS g " . $where .
...
function get_price_ladder($goods_id)
{
    ...
    $sql = "SELECT prices FROM " . $GLOBALS['xishan']->table('wholesale') .
