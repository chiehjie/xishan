...
if (!$smarty->is_cached($tpl, $cache_id))
{
   ...
   /* 根据参数生成查询语句 */
    ...
    $goods_url = $xishan->url() . "goods.php?u=$userid&id=";
    $goods = get_goods_info($goodsid);
    $goods['goods_thumb'] = (strpos($goods['goods_thumb'], 'http://') === false && strpos($goods['goods_thumb'], 'https://') === false) ? $xishan->url() . $goods['goods_thumb'] : $goods['goods_thumb'];
    $goods['goods_img'] = (strpos($goods['goods_img'], 'http://') === false && strpos($goods['goods_img'], 'https://') === false) ? $xishan->url() . $goods['goods_img'] : $goods['goods_img'];
