 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: pushers.php 17217 2011-01-19 06:29:08Z liubo $
*/
...
{
    /* 模板赋值 */
    $smarty->assign('ur_here',     '获取推送人列表');
    $smarty->assign('action_link', array('href'=>'pushers.php?act=add', 'text' => '获取推送人列表'));
    $smarty->assign('full_page',   1);
    $smarty->assign('admin_list',  get_admin_userlist());

    /* 显示页面 */
    assign_query_info();
    $smarty->display('pushers_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('pushers_list.htm'));
}
/*------------------------------------------------------ */
//-- 添加品牌
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here',    '添加推送人列表');
    $smarty->assign('action_link', array('text' => $_LANG['06_goods_brand_list'], 'href' => 'pushers.php?act=list'));
    $smarty->assign('form_action', 'insert');

    assign_query_info();
    $smarty->assign('brand', array('sort_order'=>50, 'is_show'=>1));
    $smarty->display('pushers_info.htm');
}
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$xishan->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order) ".
    ...
    $link[0]['href'] = 'pushers.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'pushers.php?act=list';
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order ".
            "FROM " .$xishan->table('brand'). " WHERE brand_id='$_REQUEST[id]'";
    $brand = $db->GetRow($sql);

    $smarty->assign('ur_here',     $_LANG['brand_edit']);
    $smarty->assign('action_link', array('text' => $_LANG['06_goods_brand_list'], 'href' => 'pushers.php?act=list&' . list_link_postfix()));
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    if ($exc->edit($param,  $_POST['id']))
    {
        ...
        $link[0]['href'] = 'pushers.php?act=list&' . list_link_postfix();
...
elseif($_REQUEST['act'] == 'add_brand')
{
    ...
    {
        $sql = "INSERT INTO " . $xishan->table('brand') . "(brand_name)" .
               "VALUES ( '$brand')";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除该品牌的图标 */
    $sql = "SELECT brand_logo FROM " .$xishan->table('brand'). " WHERE brand_id = '$id'";
    ...
    /* 更新商品的品牌编号 */
    $sql = "UPDATE " .$xishan->table('goods'). " SET brand_id=0 WHERE brand_id='$id'";
    $db->query($sql);

    $url = 'pushers.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'drop_logo')
{
    ...
    /* 取得logo名称 */
    $sql = "SELECT brand_logo FROM " .$xishan->table('brand'). " WHERE brand_id = '$brand_id'";
    $logo_name = $db->getOne($sql);

    if (!empty($logo_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/brandlogo/' .$logo_name);
        $sql = "UPDATE " .$xishan->table('brand'). " SET brand_logo = '' WHERE brand_id = '$brand_id'";
        $db->query($sql);
    }
    $link= array(array('text' => $_LANG['brand_edit_lnk'], 'href' => 'pushers.php?act=edit&id=' . $brand_id), array('text' => $_LANG['brand_list_lnk'], 'href' => 'pushers.php?act=list'));
...
function get_brandlist()
{
    ...
    if ($result === false)
    {
        ...
        {
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xishan']->table('brand') .' WHERE brand_name = \''.$_POST['brand_name'].'\'';
        }
        else
        {
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xishan']->table('brand');
        }
        ...
        if (isset($_POST['brand_name']))
        {
            ...
            $sql = "SELECT * FROM ".$GLOBALS['xishan']->table('brand')." WHERE brand_name like '%{$keyword}%' ORDER BY sort_order ASC";
        }
        else
        {
            $sql = "SELECT * FROM ".$GLOBALS['xishan']->table('brand')." ORDER BY sort_order ASC";
        }
