...
if ($_REQUEST['act'] == '')
{
    $smarty->assign('shop_url', urlencode($xishan->url()));
...
elseif ($_REQUEST['act'] == 'top')
{
    // 获得管理员设置的菜单
    $lst = array();
    $nav = $db->GetOne('SELECT nav_list FROM ' . $xishan->table('admin_user') . " WHERE user_id = '" . $_SESSION['admin_id'] . "'");
    ...
    $auth_sql = 'SELECT code, value FROM ' . $xishan->table('shop_config') . ' WHERE code = "authorize"';
	...
	$smarty->assign('admin_name',  get_admin_name());//获取管理员名称
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    'FROM ' . $xishan->table('admin_message') . ' AS a, ' . $xishan->table('admin_user') . ' AS b ' .
    ...
    /* 已完成的订单 */
    $order['finished']     = $db->GetOne('SELECT COUNT(*) FROM ' . $xishan->table('order_info').
    ...
    /* “未确认”的订单 */
    $order['unconfirmed']  = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('order_info').
    " WHERE 1 " . order_query_sql('unconfirmed'));
    $status['unconfirmed'] = OS_UNCONFIRMED;

    /* “部分发货”的订单 */
    $order['shipped_part']  = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('order_info').
...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    /* 商品信息 */
    $goods['total']   = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
    $virtual_card['total'] = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real=0 AND extension_code=\'virtual_card\'');

    $goods['new']     = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND is_new = 1 AND is_real = 1');
    $virtual_card['new']     = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND is_new = 1 AND is_real=0 AND extension_code=\'virtual_card\'');

    $goods['best']    = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND is_best = 1 AND is_real = 1');
    $virtual_card['best']    = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND is_best = 1 AND is_real=0 AND extension_code=\'virtual_card\'');

    $goods['hot']     = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND is_hot = 1 AND is_real = 1');
    $virtual_card['hot']     = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND is_hot = 1 AND is_real=0 AND extension_code=\'virtual_card\'');

    $time             = gmtime();
    $goods['promote'] = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND promote_price>0' .
    " AND promote_start_date <= '$time' AND promote_end_date >= '$time' AND is_real = 1");
    $virtual_card['promote'] = $db->GetOne('SELECT COUNT(*) FROM ' .$xishan->table('goods').
    ' WHERE is_delete = 0 AND promote_price>0' .
    ...
    if ($_CFG['use_storage'])
    {
        $sql = 'SELECT COUNT(*) FROM ' .$xishan->table('goods'). ' WHERE is_delete = 0 AND goods_number <= warn_number AND is_real = 1';
        $goods['warn'] = $db->GetOne($sql);
        $sql = 'SELECT COUNT(*) FROM ' .$xishan->table('goods'). ' WHERE is_delete = 0 AND goods_number <= warn_number AND is_real=0 AND extension_code=\'virtual_card\'';
    ...
    /* 访问统计信息 */
    $today  = local_getdate();
    $sql    = 'SELECT COUNT(*) FROM ' .$xishan->table('stats').
    ...
    "FROM " . $xishan->table('feedback') . " AS f ".
    "LEFT JOIN " . $xishan->table('feedback') . " AS r ON r.parent_id=f.msg_id " .
    'WHERE f.parent_id=0 AND ISNULL(r.msg_id) ' ;
    $smarty->assign('feedback_number', $db->GetOne($sql));

    /* 未审核评论 */
    $smarty->assign('comment_number', $db->getOne('SELECT COUNT(*) FROM ' . $xishan->table('comment') .
    ...
    /* 缺货登记 */
    $smarty->assign('booking_goods', $db->getOne('SELECT COUNT(*) FROM ' . $xishan->table('booking_goods') . ' WHERE is_dispose = 0'));

    /* 退款申请 */
    $smarty->assign('new_repay', $db->getOne('SELECT COUNT(*) FROM ' . $xishan->table('user_account') . ' WHERE process_type = ' . SURPLUS_RETURN . ' AND is_paid = 0 '));
...
elseif ($_REQUEST['act'] == 'main_api')
{
    ...
    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
        ...
        $ecs_user = $db->getOne('SELECT COUNT(*) FROM ' . $xishan->table('users'));
        $ecs_template = $db->getOne('SELECT value FROM ' . $xishan->table('shop_config') . ' WHERE code = \'template\'');
        $style = $db->getOne('SELECT value FROM ' . $xishan->table('shop_config') . ' WHERE code = \'stylename\'');
        if($style == '')
        {
            $style = '0';
        }
        $ecs_style = $style;
        $shop_url = urlencode($xishan->url());
...
elseif ($_REQUEST['act'] == 'first')
{
    ...
    $sql = 'SELECT value from ' . $xishan->table('shop_config') . " WHERE code='shop_name'";
    $shop_name = $db->getOne($sql);

    $smarty->assign('shop_name', $shop_name);

    $sql = 'SELECT value from ' . $xishan->table('shop_config') . " WHERE code='shop_title'";
...
elseif ($_REQUEST['act'] == 'second')
{
    ...
    if(!empty($shop_name))
    {
        $sql = 'UPDATE ' . $xishan->table('shop_config') . " SET value = '$shop_name' WHERE code = 'shop_name'";
        $db->query($sql);
    }

    if(!empty($shop_title))
    {
        $sql = 'UPDATE ' . $xishan->table('shop_config') . " SET value = '$shop_title' WHERE code = 'shop_title'";
        $db->query($sql);
    }

    if(!empty($shop_address))
    {
        $sql = 'UPDATE ' . $xishan->table('shop_config') . " SET value = '$shop_address' WHERE code = 'shop_address'";
        $db->query($sql);
    }

    if(!empty($shop_country))
    {
        $sql = 'UPDATE ' . $xishan->table('shop_config') . "SET value = '$shop_country' WHERE code='shop_country'";
        $db->query($sql);
    }

    if(!empty($shop_province))
    {
        $sql = 'UPDATE ' . $xishan->table('shop_config') . "SET value = '$shop_province' WHERE code='shop_province'";
        $db->query($sql);
    }

    if(!empty($shop_city))
    {
        $sql = 'UPDATE ' . $xishan->table('shop_config') . "SET value = '$shop_city' WHERE code='shop_city'";
        $db->query($sql);
    }
    ...
    if(!empty($shipping))
    {
        ...
        $sql = "SELECT shipping_id FROM " .$xishan->table('shipping'). " WHERE shipping_code = '$shipping'";
        $shipping_id = $db->GetOne($sql);

        if($shipping_id <= 0)
        {
            $insure = empty($modules[0]['insure']) ? 0 : $modules[0]['insure'];
            $sql = "INSERT INTO " . $xishan->table('shipping') . " (" .
        ...
        if(!empty($area_name))
        {
            $sql = "SELECT shipping_area_id FROM " .$xishan->table("shipping_area").
            ...
            if($area_id <= 0)
            {
                ...
                $sql = "INSERT INTO " .$xishan->table('shipping_area').
            ...
            /* 添加选定的城市和地区 */
            $sql = "REPLACE INTO ".$xishan->table('area_region')." (shipping_area_id, region_id) VALUES ('$area_id', '$region_id')";
            $db->query($sql);
        }
    }
    ...
    if(!empty($payment))
    {
        ...
        /* 安装，检查该支付方式是否曾经安装过 */
        $sql = "SELECT COUNT(*) FROM " . $xishan->table('payment') . " WHERE pay_code = '$payment'";
        if ($db->GetOne($sql) > 0)
        {
            $sql = "UPDATE " . $xishan->table('payment') .
...
//            $modules = read_modules('../includes/modules/payment');
            ...
            $sql = "INSERT INTO " . $xishan->table('payment') . " (pay_code, pay_name, pay_desc, pay_config, is_cod, pay_fee, enabled, is_online)" .
...
elseif ($_REQUEST['act'] == 'third')
{
    ...
    if(!empty($good_brand))
    {
        $sql = 'INSERT INTO ' . $xishan->table('brand') . " (brand_name, is_show)" .
        " values('" . $good_brand . "', '1')";
        $db->query($sql);
    ...
    if(!empty($good_category))
    {
        $sql = 'INSERT INTO ' . $xishan->table('category') . " (cat_name, parent_id, is_show)" .
        ...
        $max_id     = $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$xishan->table('goods'));
        ...
        if(!empty($good_name))
        {
            ...
            $sql = 'INSERT INTO ' . $xishan->table('goods') . "(goods_name, goods_sn, goods_number, cat_id, brand_id, goods_brief, shop_price, market_price, goods_img, goods_thumb, original_img,add_time, last_update,
...
elseif ($_REQUEST['act'] == 'check_order')
{
    ...
    /* 新订单 */
    $sql = 'SELECT COUNT(*) FROM ' . $xishan->table('order_info').
...
elseif ($_REQUEST['act'] == 'save_todolist')
{
    ...
    $sql = "UPDATE" .$GLOBALS['xishan']->table('admin_user'). " SET todolist='" . $content . "' WHERE user_id = " . $_SESSION['admin_id'];
    $GLOBALS['db']->query($sql);
}

elseif ($_REQUEST['act'] == 'get_todolist')
{
    $sql     = "SELECT todolist FROM " .$GLOBALS['xishan']->table('admin_user'). " WHERE user_id = " . $_SESSION['admin_id'];
...
elseif ($_REQUEST['act'] == 'send_mail')
{
    ...
    $sql = "SELECT * FROM " . $xishan->table('email_sendlist') . " ORDER BY pri DESC, last_send ASC LIMIT 1";
    $row = $db->getRow($sql);
    ...
    {
        $sql = "DELETE FROM " . $xishan->table('email_sendlist') . " WHERE id = '$row[id]'";
        $db->query($sql);
        $count = $db->getOne("SELECT COUNT(*) FROM " . $xishan->table('email_sendlist'));
        make_json_result('', $_LANG['mailsend_skip'], array('count' => $count, 'goon' => 1));
    }

    //查询相关模板
    $sql = "SELECT * FROM " . $xishan->table('mail_templates') . " WHERE template_id = '$row[template_id]'";
    $rt = $db->getRow($sql);
    ...
    if ($rt['template_id'] && $rt['template_content'])
    {
        ...
        else
        {
            ...
            {
                $time = time();
                $sql = "UPDATE " . $xishan->table('email_sendlist') . " SET error = error + 1, pri = 0, last_send = '$time' WHERE id = '$row[id]'";
            }
            else
            {
                //将出错超次的纪录删除
 ...
 */
 function get_admin_name(){
	 $list = array();
    $sql  = "select * from ecs_admin_user where user_id='$_SESSION[admin_id]'";
