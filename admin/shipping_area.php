...
$exc = new exchange($xishan->table('shipping_area'), $db, 'shipping_area_id', 'shipping_area_name');
...
elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping']))
{
    ...
    $shipping = $db->getRow("SELECT shipping_name, shipping_code FROM " .$xishan->table('shipping'). " WHERE shipping_id='$_REQUEST[shipping]'");
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$xishan->table("shipping_area").
    ...
    {
        $shipping_code = $db->getOne("SELECT shipping_code FROM " .$xishan->table('shipping').
        ...
        if (isset($_POST['regions']) && is_array($_POST['regions']))
        {
            ...
            {
                $sql = "INSERT INTO ".$xishan->table('area_region')." (shipping_area_id, region_id) VALUES ('$new_id', '$val')";
                $db->query($sql);
            }
        }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* ".
            "FROM " .$xishan->table('shipping'). " AS a, " .$xishan->table('shipping_area'). " AS b ".
...
//       }
    ...
    $sql = "SELECT a.region_id, r.region_name ".
            "FROM ".$xishan->table('area_region')." AS a, ".$xishan->table('region'). " AS r ".
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    {
        $shipping_code = $db->getOne("SELECT shipping_code FROM " .$xishan->table('shipping'). " WHERE shipping_id='$_POST[shipping]'");
        $plugin        = '../includes/modules/shipping/'. $shipping_code. ".php";
        ...
        $sql = "UPDATE " .$xishan->table('shipping_area').
        ...
        $sql = "SELECT region_id, parent_id FROM " . $xishan->table('region');
        $res = $db->query($sql);
        ...
        /* 清除原有的城市和地区 */
        $db->query("DELETE FROM ".$xishan->table("area_region")." WHERE shipping_area_id='$_POST[id]'");

        /* 添加选定的城市和地区 */
        foreach ($selected_regions AS $key => $val)
        {
            $sql = "INSERT INTO ".$xishan->table('area_region')." (shipping_area_id, region_id) VALUES ('$_POST[id]', '$val')";
            $db->query($sql);
        }
...
elseif ($_REQUEST['act'] == 'multi_remove')
{
    ...
    if (isset($_POST['areas']) && count($_POST['areas']) > 0)
    {
        ...
        {
            $db->query("DELETE FROM " .$xishan->table('shipping_area'). " WHERE shipping_area_id='$v'");
            $i++;
        }
...
elseif ($_REQUEST['act'] == 'remove_area')
{
    ...
    $exc->drop($id);
    $db->query('DELETE FROM '.$xishan->table('area_region').' WHERE shipping_area_id='.$id);
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xishan']->table('shipping_area');
    if ($shipping_id > 0)
    {
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $sql = "SELECT r.region_name " .
                "FROM " . $GLOBALS['xishan']->table('area_region'). " AS a, " .
                    $GLOBALS['xishan']->table('region') . " AS r ".
