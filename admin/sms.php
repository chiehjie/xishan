...
switch ($action)
{
    ...
    case 'display_send_ui' :
      ...
      case 'sms_sign':
        ...
        if ($sms->has_registered())
        {
            $sql="SELECT * FROM ". $xishan->table('shop_config') . "WHERE  code='sms_sign'";
            $row=$db->getRow($sql);
            if(!empty($row['id']))
            {
            ...
            $sql="SELECT * FROM ". $xishan->table('shop_config') . "WHERE  code='default_sms_sign'";
    ...
    case 'send_sms' :
        ...
        if ($send_rank != 0)
        {
            ...
            if($rank_array['0'] == 1)
            {
                $sql = 'SELECT mobile_phone FROM ' . $xishan->table('users') . "WHERE mobile_phone <>'' ";
                $row = $db->query($sql);
            ...
            {
                $rank_sql = "SELECT * FROM " . $xishan->table('user_rank') . " WHERE rank_id = '" . $rank_array['1'] . "'";
                $rank_row = $db->getRow($rank_sql);
                //$sql = 'SELECT mobile_phone FROM ' . $xishan->table('users') . "WHERE mobile_phone <>'' AND rank_points > " .$rank_row['min_points']." AND rank_points < ".$rank_row['max_points']." ";

                if($rank_row['special_rank']==1) 
                {
                    $sql = 'SELECT mobile_phone FROM ' . $xishan->table('users') . " WHERE mobile_phone <>'' AND user_rank = '" . $rank_array['1'] . "'";
                }
                else
                {
                    $sql = 'SELECT mobile_phone FROM ' . $xishan->table('users') . "WHERE mobile_phone <>'' AND rank_points > " .$rank_row['min_points']." AND rank_points < ".$rank_row['max_points']." ";
...
{
	$sql="SELECT `id` FROM ".$GLOBALS['xishan']->table(shop_config)." WHERE `code`='$config_code'";
	$c_node_id=$GLOBALS['db']->getOne($sql);
	if(empty($c_node_id))
    {
    	for ($i=247;$i<=270;$i++)
        {
        	$sql="SELECT `id` FROM ".$GLOBALS['xishan']->table(shop_config)." WHERE `id`='$i'";
        	$c_id=$GLOBALS['db']->getOne($sql);
        	if(empty($c_id))
            {
            	$sql="INSERT INTO ".$GLOBALS['xishan']->table(shop_config)."(`id`,`parent_id`,`code`,`type`,`value`,`sort_order`) VALUES ('$i','2','$config_code','hidden','$config_value','1')";
    ...
    {
    	$sql="UPDATE ".$GLOBALS['xishan']->table(shop_config)." SET `value`='$config_value'  WHERE `code`='$config_code'";
    	$GLOBALS['db']->query($sql);
    }
}
