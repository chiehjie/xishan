...
$exc   = new exchange($xishan->table("my_pusher"), $db, 'pusher_id', 'pusher_name');
...
{
    // echo "1";
    admin_priv('my_pusher_list');
    $pid = !empty($_REQUEST['pid']) ? intval($_REQUEST['pid']) : 0;

    $smarty->assign('ur_here',     $_LANG['pusher_add']);
    $smarty->assign('action_link', array('text' => $_LANG['pusher_add'], 'href' => 'my_pusher.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('my_pusher_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('my_pusher_list.htm'), '',
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here',       $_LANG['adspusher_list_add']);
    $smarty->assign('action_link',   array('href' => 'my_pusher.php?act=list', 'text' => $_LANG['pusher_list']));
    ...
    assign_query_info();
    $smarty->display('my_pusher_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $pusher_name = !empty($_POST['pusher_name']) ? trim($_POST['pusher_name']) : '';
    $pusher_phone = !empty($_POST['pusher_phone']) ? trim($_POST['pusher_phone']) : '';
    $pusher_account = !empty($_POST['pusher_account']) ? trim($_POST['pusher_account']) : '';
    // var_dump($pusher_phone);exit;
    ...
    /* 查看广告名称是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$xishan->table('my_pusher'). " WHERE pusher_name = '$pusher_name'";
    // var_dump($sql);exit;
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$xishan->table('my_pusher'). " (pusher_name,pusher_phone,pusher_account)
    VALUES ('$_POST[pusher_name]',
            '$_POST[pusher_phone]',
            '$_POST[pusher_account]')";
    // var_dump($sql);exit;
    $db->query($sql);
    /* 记录管理员操作 */
    admin_log($_POST['pusher_name'], 'add', 'ads');
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告数据 */
    $sql = "SELECT * FROM " .$xishan->table('my_pusher'). " WHERE pusher_id='".intval($_REQUEST['id'])."'";
    // var_dump($sql);exit;
    $ads_arr = $db->getRow($sql);
    // var_dump($ads_arr);exit;
    $ads_arr['pusher_name'] = htmlspecialchars($ads_arr['pusher_name']);

    /* 格式化广告的有效日期 */
    $ads_arr['pusher_account']  = htmlspecialchars($ads_arr['pusher_account']);
    $ads_arr['pusher_phone']    = htmlspecialchars($ads_arr['pusher_phone']);
    $ads_arr['pusher_id']    = htmlspecialchars($ads_arr['pusher_id']);
    ...
    $smarty->assign('action_link',   array('href' => 'my_pusher.php?act=list', 'text' => $_LANG['ad_list']));
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$xishan->table('my_pusher'). " SET ".
            "pusher_name = '$_POST[pusher_name]', ".
            "pusher_account     = '$_POST[pusher_account]', ".
            "pusher_phone     = '$_POST[pusher_phone]' ".
            "WHERE pusher_id = '$id'";
            // var_dump($sql);exit;
    $db->query($sql);

   /* 记录管理员操作 */
   admin_log($_POST['pusher_name'], 'edit', 'ads');

   clear_cache_files(); // 清除模版缓存

   /* 提示信息 */
   $href[] = array('text' => $_LANG['back_ads_list'], 'href' => 'my_pusher.php?act=list');
   sys_msg($_LANG['edit'] .' '.$_POST['pusher_name'].' '. $_LANG['attradd_succed'], 0, $href);
...
//-- 编辑广告名称
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_pusher_name')
{
    check_authz_json('ad_manage');

    $id      = intval($_POST['id']);
    $pusher_name = json_str_iconv(trim($_POST['val']));

    /* 检查广告名称是否重复 */
    if ($exc->num('ppusher_name', $pusher_name, $id) != 0)
    {
        make_json_error(sprintf($_LANG['pusher_name_exist'], $payee_name));
    }
    else
    {
        if ($exc->edit("pusher_name = '$pusher_name'", $id))
        {
            admin_log($pusher_name,'edit','ads');
            make_json_result(stripslashes($pusher_name));
        }
        else
        {
...
function get_adslist()
{
    ...
    $filter = array();
    $filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'ad.pusher_name' : trim($_REQUEST['sort_by']);
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['xishan']->table('my_pusher'). ' AS ad ' . $where;
