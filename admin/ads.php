...
$exc   = new exchange($xishan->table("ad"), $db, 'ad_id', 'ad_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 查看广告名称是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$xishan->table('ad'). " WHERE ad_name = '$ad_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$xishan->table('ad'). " (position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled)
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告数据 */
    $sql = "SELECT * FROM " .$xishan->table('ad'). " WHERE ad_id='".intval($_REQUEST['id'])."'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    /* 更新信息 */
    $sql = "UPDATE " .$xishan->table('ad'). " SET ".
...
function get_adslist()
{
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['xishan']->table('ad'). ' AS ad ' . $where;
    ...
    $sql = 'SELECT ad.*, COUNT(o.order_id) AS ad_stats, p.position_name '.
            'FROM ' .$GLOBALS['xishan']->table('ad'). 'AS ad ' .
            'LEFT JOIN ' . $GLOBALS['xishan']->table('ad_position'). ' AS p ON p.position_id = ad.position_id '.
            'LEFT JOIN ' . $GLOBALS['xishan']->table('order_info'). " AS o ON o.from_ad = ad.ad_id $where " .
            'GROUP BY ad.ad_id '.
