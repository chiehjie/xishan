...
else
{
    ...
    $db->query("UPDATE " .$xishan->table('shop_config'). " SET VALUE='$config' WHERE code='sitemap'");

    /* 商品分类 */
    $sql = "SELECT cat_id,cat_name FROM " .$xishan->table('category'). " ORDER BY parent_id";
    $res = $db->query($sql);
    ...
    /* 文章分类 */
    $sql = "SELECT cat_id,cat_name FROM " .$xishan->table('article_cat'). " WHERE cat_type=1";
    $res = $db->query($sql);
    ...
    /* 商品 */
    $sql = "SELECT goods_id, goods_name FROM " .$xishan->table('goods'). " WHERE is_delete = 0";
    $res = $db->query($sql);
    ...
    /* 文章 */
    $sql = "SELECT article_id,title,file_url,open_type FROM " .$xishan->table('article'). " WHERE is_open=1";
    $res = $db->query($sql);
    ...
    if ($sm->build($sm_file))
    {
        sys_msg(sprintf($_LANG['generate_success'], $xishan->url()."sitemaps.xml"));
    }
    else
    {
        ...
        if ($sm->build($sm_file))
        {
            sys_msg(sprintf($_LANG['generate_success'], $xishan->url(). DATA_DIR . '/sitemaps.xml'));
        }
        else
        {
