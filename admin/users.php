...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = "SELECT rank_id, rank_name, min_points FROM ".$xishan->table('user_rank')." ORDER BY min_points ASC ";
    $rs = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    /* 取出注册扩展字段 */
    $sql = 'SELECT * FROM ' . $xishan->table('reg_fields') . ' WHERE type < 2 AND display = 1 AND id != 6 ORDER BY dis_order, id';
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $user_type = in_array($user_type, array(0,1,2,3)) ? $user_type: 0;
    ...
    $other['user_type']=$user_type;
	  $other['admin_id']="$_SESSION[admin_id]";
	    $img_name = basename($image->upload_image($_FILES['user_head'],'user_head'));
  $other['user_head']= $img_name;
    

    $db->autoExecute($xishan->table('users'), $other, 'UPDATE', "user_name = '$username' ");
    /* 记录管理员操作 */
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.phone,u.user_type,u.user_head,u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone".
        " FROM " .$xishan->table('users'). " u LEFT JOIN " . $xishan->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
    ...
    $sql = "SELECT u.phone,u.user_type,u.user_head,u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if(!empty($password))
    {
            $sql="UPDATE ".$xishan->table('users'). "SET `ec_salt`='0' WHERE user_name= '".$username."'";
            $db->query($sql);
    }
    /* 更新用户扩展字段的数据 */
    $sql = 'SELECT id FROM ' . $xishan->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有扩展字段的id
    ...
    $user_id = $user_id_arr['user_id'];
     ...
     $img_name = basename($image->upload_image($_FILES['user_head'],'user_head'));
        ...
        $sql = "UPDATE ecs_users SET ".
               "user_head='$img_name' where user_id=".$user_id;
   ...
   $db->query($sql);
    ...
    $db->autoExecute($xishan->table('users'), $other, 'UPDATE', "user_name = '$username'");
    // var_dump($xishan->table('users'), $other, 'UPDATE', "user_name = '$username'");
    // exit;
...
{
    /* 检查权限 */
    admin_priv('user_delete');

    if (isset($_POST['checkboxes']))
    {
        $sql = "SELECT user_name FROM " . $xishan->table('users') . " WHERE user_id " . db_create_in($_POST['checkboxes']);
        $col = $db->getCol($sql);
...
elseif ($_REQUEST['act'] == 'edit_username')
{
    ...
    if ($users->edit_user($id, $username))
    {
        ...
        {
            /* 更新商城会员表 */
            $db->query('UPDATE ' .$xishan->table('users'). " SET user_name = '$username' WHERE user_id = '$id'");
        }
...
elseif ($_REQUEST['act'] == 'edit_email')
{
    ...
    $sql = "SELECT user_name FROM " . $xishan->table('users') . " WHERE user_id = '$id'";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT user_name FROM " . $xishan->table('users') . " WHERE user_id = '" . $_GET['id'] . "'";
    $username = $db->getOne($sql);
    /* 通过插件来删除用户 */
    $users = init_users();
...
elseif ($_REQUEST['act'] == 'address_list')
{
    ...
    $sql = "SELECT a.*, c.region_name AS country_name, p.region_name AS province, ct.region_name AS city_name, d.region_name AS district_name ".
           " FROM " .$xishan->table('user_address'). " as a ".
           " LEFT JOIN " . $xishan->table('region') . " AS c ON c.region_id = a.country " .
           " LEFT JOIN " . $xishan->table('region') . " AS p ON p.region_id = a.province " .
           " LEFT JOIN " . $xishan->table('region') . " AS ct ON ct.region_id = a.city " .
           " LEFT JOIN " . $xishan->table('region') . " AS d ON d.region_id = a.district " .
           " WHERE user_id='$id'";
...
elseif ($_REQUEST['act'] == 'remove_parent')
{
    ...
    $sql = "UPDATE " . $xishan->table('users') . " SET parent_id = 0 WHERE user_id = '" . $_GET['id'] . "'";
    $db->query($sql);
    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        if ($count)
        {
            $sql = "SELECT user_id, user_name, '$i' AS level, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
                    " FROM " . $GLOBALS['xishan']->table('users') . " WHERE user_id IN($up_uid)" .
...
if ($_SESSION['admin_id']=='1') {
    ...
    $sql="delete from ecs_users where user_id=$user_id";
    $GLOBALS['db']->query($sql);
    $link[] = array('href' => 'users.php?act=list', 'text' => '跳转到会员列表页面');
    sys_msg('删除成功', 0, $link);
}
else
{
    ...
    $sql="UPDATE ecs_users SET admin_id ='1' WHERE user_id=$user_id";
    $GLOBALS['db']->query($sql);
    $link[] = array('href' => 'users.php?act=list', 'text' => '跳转到会员列表页面');
    sys_msg('删除成功', 0, $link);
}
...
elseif ($_REQUEST['act'] == 'alist')
{
 /* 模板赋值 */
   admin_priv('my_users_alist'); 

    $smarty->assign('users_alist',  get_users_alist());
    
    /* 显示页面 */
    assign_query_info();
    $smarty->display('users_alist.htm');
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['rank'])
        {
            $sql = "SELECT min_points, max_points, special_rank FROM ".$GLOBALS['xishan']->table('user_rank')." WHERE rank_id = '$filter[rank]'";
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['xishan']->table('users') . $ex_where);


        /* 分页大小 */
        $filter = page_and_size($filter);
        if ($_SESSION['admin_id']=='1') {
             $sql = "SELECT * FROM ecs_users,ecs_user_role"  . $ex_where .' and ecs_users.user_type=ecs_user_role.user_type '.
                " ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
        }else
        {
             $sql = "SELECT * FROM ecs_users,ecs_user_role"  . $ex_where . "and ecs_users.user_type=ecs_user_role.user_type and admin_id=$_SESSION[admin_id]".
                " ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
        }
...
function get_users_alist(){
     ...
     if (isset($_REQUEST['is_ajax']) && $_REQUEST['is_ajax'] == 1)
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['xishan']->table('users') . $ex_where);
     $filter = page_and_size($filter);
    $sql  = 'select * from ecs_users where admin_id=1' .
    " ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] .
                " LIMIT " . $filter['start'] . ',' . $filter['page_size'];
                $filter['keywords'] = stripslashes($filter['keywords']);
        set_filter($filter, $sql);
