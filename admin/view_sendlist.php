...
{
    $id = (int)$_REQUEST['id'];
    $sql = "DELETE FROM " . $GLOBALS['xishan']->table('email_sendlist') . " WHERE id = '$id' LIMIT 1";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'batch_remove')
{
    ...
    {
        $sql = "DELETE FROM " . $xishan->table('email_sendlist') . " WHERE id " . db_create_in($_POST['checkboxes']);
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'batch_send')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        $sql = "SELECT * FROM " . $xishan->table('email_sendlist') . "WHERE id " . db_create_in($_POST['checkboxes']) . " ORDER BY pri DESC, last_send ASC LIMIT 1";
        $row = $db->getRow($sql);
        ...
        $sql = "SELECT * FROM " . $xishan->table('email_sendlist') . "WHERE id " . db_create_in($_POST['checkboxes']) . " ORDER BY pri DESC, last_send ASC";
        $res = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'all_send')
{
    ...
    $sql = "SELECT * FROM " . $xishan->table('email_sendlist') . " ORDER BY pri DESC, last_send ASC";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
        //发送列表不为空，邮件地址为空
...
function get_sendlist()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT count(*) FROM " . $GLOBALS['xishan']->table('email_sendlist') . " e LEFT JOIN " . $GLOBALS['xishan']->table('mail_templates') . " m ON e.template_id = m.template_id";
        ...
        /* 查询 */
        $sql = "SELECT e.id, e.email, e.pri, e.error, FROM_UNIXTIME(e.last_send) AS last_send, m.template_subject, m.type FROM " . $GLOBALS['xishan']->table('email_sendlist') . " e LEFT JOIN " . $GLOBALS['xishan']->table('mail_templates') . " m ON e.template_id = m.template_id" .
