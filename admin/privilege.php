...
/* 初始化 $exc 对象 */
$exc = new exchange($xishan->table("admin_user"), $db, 'user_id', 'user_name');
...
if ($_REQUEST['act'] == 'logout')
{
    ...
    $url = $GLOBALS['xishan']->url()."admin/privilege.php?act=login";
...
if ($_REQUEST['act'] == 'login')
{
    ...
    if($_REQUEST['type']=='yunqi'){
                ...
                if ($admin_row){
                    ...
                    if(empty($row['ec_salt']))
                    {
                        ...
                        $new_possword=md5(md5($user['password']).$ec_salt);
                         $db->query("UPDATE " .$xishan->table('admin_user').
                    ...
                    if($certificate['certificate_id']&&$certificate['token']&&$certificate['node_id']){
                        ...
                        $auth_sql = 'SELECT code FROM ' . $xishan->table('shop_config') . ' WHERE code = "authorize"';
                        if ($info['status']=='success' && $info['data']['service']['authorize_code'] == 'NCH' && !$db->getRow($auth_sql)) {
                            ...
                            $db->query("INSERT " . $xishan->table('shop_config') . " set parent_id=2, code='authorize', type='hidden', value='" . serialize($auth_conf) . "'");
                        }
                    }
                    // 清除购物车中过期的数据
                    clear_cart();
        ...
        $activate_callback = $GLOBALS['xishan']->url()."admin/certificate.php?act=get_certificate&type=index";
        $activate_iframe_url = $cert->get_authorize_url($activate_callback);
        $smarty->assign('activate_iframe_url',$activate_iframe_url);
        
        $callback = $GLOBALS['xishan']->url()."admin/privilege.php?act=login&type=yunqi";
...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    $sql="SELECT `ec_salt` FROM ". $xishan->table('admin_user') ."WHERE user_name = '" . $_POST['username']."'";
    ...
    if ($row)
    {
        ...
        //如果是云起认证，则使用云起账号登录
        if($row['passport_uid']){
            $yunqi_login = $xishan->url."privilege.php?act=login&type=yunqi";
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        $sql = $sql = "SELECT * FROM " . $xishan->table('admin_user') . " WHERE ecs_admin_user.cat_id = '".$_POST['category']."'";;
    ...
    {
        $sql = "SELECT action_list FROM " . $xishan->table('role') . " WHERE role_id = '".$_POST['select_role']."'";
        $row = $db->getRow($sql);
...
// if (empty($_POST['select_role']) {
//     echo "<alert>'请选择角色'</alert>";
//     # code...
// }else{
    
// }
        $sql = "SELECT nav_list FROM " . $xishan->table('admin_user') . " WHERE action_list = 'all'";
        $row = $db->getRow($sql);

          /*处理图片*/
    $img_name = basename($image->upload_image($_FILES['admin_head'],'admin_head'));

    $sql = "INSERT INTO ".$xishan->table('admin_user')." (admin_name,user_name, phone,admin_head, password, add_time, nav_list, action_list, role_id, cat_id) ".
           "VALUES ('$_POST[admin_name]','".trim($_POST['user_name'])."', '$_POST[phone]','$img_name', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id', '$cat_id')";
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    {
        admin_priv('my_admin_edit');
    }

    /* 获取管理员信息 */
    $sql = "SELECT * FROM " .$xishan->table('admin_user').
    ...
    {
        $sql = "SELECT agency_name FROM " . $xishan->table('agency') . " WHERE agency_id = '$user_info[agency_id]'";
    ...
    $smarty->assign('select_category',  get_category_list());
    /* 获得该管理员的权限 */
    $priv_str = $db->getOne("SELECT action_list FROM " .$xishan->table('admin_user'). " WHERE user_id = '$_GET[id]'");
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    {
        /* 查询旧密码并与输入的旧密码比较是否相同 */
        $sql = "SELECT password FROM ".$xishan->table('admin_user')." WHERE user_id = '$admin_id'";
        $old_password = $db->getOne($sql);
		$sql ="SELECT ec_salt FROM ".$xishan->table('admin_user')." WHERE user_id = '$admin_id'";
    ...
    if (!empty($_POST['category']))
    {
       $sql = $sql = "SELECT * FROM " . $xishan->table('admin_user') . " WHERE cat_id = '".$_POST['category']."'";;
        $row = $db->getRow($sql);
        $cat_id = ', cat_id = '.$_POST['category'].' ';
        // var_dump($cat_id);exit;
    }
    //更新管理员信息
    ...
    if($pwd_modified)
    {
        $sql = "UPDATE " .$xishan->table('admin_user'). " SET ".
    ...
    else
    {
       $img_name = basename($image->upload_image($_FILES['admin_head'],'admin_head'));
        ...
        $sql = "UPDATE " .$xishan->table('admin_user'). " SET ".
               "user_name = '$admin_name',admin_name='$_POST[admin_name]',phone='$_POST[phone]',admin_head='$img_name' ".
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    /* 包含插件菜单语言项 */
    $sql = "SELECT code FROM ".$xishan->table('plugins');
    $rs = $db->query($sql);
    ...
    $sql = "SELECT user_id, user_name, email, nav_list ".
           "FROM " .$xishan->table('admin_user'). " WHERE user_id = '".$_SESSION['admin_id']."'";
    $user_info = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'allot')
{
    ...
    /* 获取权限的分组数据 */
    $sql_query = "SELECT action_id, parent_id, action_code,relevance FROM " .$xishan->table('admin_action').
                 " WHERE parent_id = 0";
    ...
    /* 按权限组查询底级的权限名称 */
    $sql = "SELECT action_id, parent_id, action_code,relevance FROM " .$xishan->table('admin_action').
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    /* 取得当前管理员用户名 */
    $admin_name = $db->getOne("SELECT user_name FROM " .$xishan->table('admin_user'). " WHERE user_id = '$_POST[id]'");

    /* 更新管理员的权限 */
    $act_list = @join(",", $_POST['action_code']);
    $sql = "UPDATE " .$xishan->table('admin_user'). " SET action_list = '$act_list', role_id = '' ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 获得管理员用户名 */
    $admin_user_info = $db->getRow('SELECT user_name,passport_uid FROM '.$xishan->table('admin_user')." WHERE user_id='$id'");
...
{
    $list = array();
    $sql  = 'select * from ecs_admin_user,ecs_role where ecs_admin_user.role_id=ecs_role.role_id  ';
...
function clear_cart()
{
    ...
    $sql = "SELECT DISTINCT session_id " .
            "FROM " . $GLOBALS['xishan']->table('cart') . " AS c, " .
                $GLOBALS['xishan']->table('sessions') . " AS s " .
...
function get_role_list()
{
    ...
    $sql  = 'SELECT role_id, role_name, action_list '.
            'FROM ' .$GLOBALS['xishan']->table('role');
    $list = $GLOBALS['db']->getAll($sql);
    return $list;
}
...
function get_category_list()
{
    ...
    if ($_SESSION['admin_id']=='1') {
            ...
            $sql="select cat_name from ecs_admin_user,ecs_category where
            ecs_admin_user.cat_id  = ecs_category.cat_id";
       }
