...
{
    $tables = $db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($xishan->prefix) . "%'");
...
if ($_REQUEST['act'] == 'dumpsql')
{
    ...
    {
        case 'full':
            $except = array($xishan->prefix.'sessions', $xishan->prefix.'sessions_data');
            $temp = $db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($xishan->prefix) . "%'");
...
if ($_REQUEST['act'] == 'optimize')
{
    ...
    $db_ver = $db_ver_arr;
    $ret = $db ->query("SHOW TABLE STATUS LIKE '" . mysql_like_quote($xishan->prefix) . "%'");
...
if($_REQUEST['act']=='cleardata'){
    ...
    if($_SESSION['yunqi_login'] or tryLogin($data,$msg)){
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('goods'));
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('goods_attr'));
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('goods_cat'));
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('order_info'));
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('order_goods'));
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('delivery_goods'));
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('delivery_order'));
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('back_order'));
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('order_action'));
        $GLOBALS['db']->query("truncate table ".$GLOBALS['xishan']->table('category'));
