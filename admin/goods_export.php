...
elseif ($_REQUEST['act'] == 'act_export_taobao')
{
    ...
    {
        $sql = "SELECT region_id,  region_name FROM " . $xishan->table('region') . " WHERE region_id IN ('$_CFG[shop_province]',  '$_CFG[shop_city]')";
        $arr = $db->getAll($sql);
    ...
    " FROM " . $xishan->table('goods') . " AS g ". $where;
...
elseif($_REQUEST['act'] == 'act_export_ecshop')
{
    ...
    $sql = "SELECT g.*, b.brand_name as brandname " .
           " FROM " . $xishan->table('goods') . " AS g LEFT JOIN " . $xishan->table('brand') . " AS b " .
...
elseif ($_REQUEST['act'] == 'act_export_custom')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $sql = "SELECT `attr_id`, `attr_value` FROM " . $xishan->table('goods_attr') . " WHERE `goods_id` = '" . $row['goods_id'] . "'";
        $query = $db->query($sql);
...
{
    $sql = "SELECT `attr_id`, `cat_id`, `attr_name` FROM " . $GLOBALS['xishan']->table('attribute') . " ";
    if (!empty($cat_id))
    {
...
function set_goods_field_name($array, $lang)
{
    ...
    foreach ($array as $key => $value)
    {
        ...
        {
            $tmp_fields[$key] = $GLOBALS['db']->getOne("SELECT `attr_name` FROM " . $GLOBALS['xishan']->table('attribute') . " WHERE `attr_id` = '" . intval($value) . "'");
        }
    }
    return $tmp_fields;
}
