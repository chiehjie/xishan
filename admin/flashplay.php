...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    {
        $sql = "UPDATE " .$GLOBALS['xishan']->table('shop_config'). " SET value = '$flash_theme' WHERE code = 'flash_theme'";
...
elseif ($_REQUEST['act'] == 'custom_insert')
{
    ...
    $db->autoExecute($xishan->table('ad_custom'), $ad, 'INSERT', '', 'SILENT');
    $ad_id = $db->insert_id();
...
elseif($_REQUEST['act']== 'custom_del')
{
    ...
    $query = $db->query("DELETE FROM " . $xishan->table('ad_custom') . " WHERE ad_id = $id");
...
elseif ($_REQUEST['act'] == 'custom_status')
{
    ...
    if (modfiy_ad_status($id, $ad_status))
    {
        ...
        /* 标签初始化 */
        $sql = "SELECT  value FROM " . $xishan->table("shop_config") . " WHERE id =337";
...
elseif ($_REQUEST['act'] == 'custom_edit')
{
    ...
    /* 查询自定义广告信息 */
    $sql = "SELECT ad_id, ad_type, content, url, ad_status, ad_name FROM " . $GLOBALS['xishan']->table("ad_custom") . " WHERE ad_id = $id LIMIT 0, 1";
...
elseif ($_REQUEST['act'] == 'custom_update')
{
    ...
    /* 查询自定义广告信息 */
    $sql = "SELECT ad_id, ad_type, content, url, ad_status, ad_name FROM " . $GLOBALS['xishan']->table("ad_custom") . " WHERE ad_id = " . $filter['ad']['id'] ." LIMIT 0, 1";
    ...
    $db->autoExecute($xishan->table('ad_custom'), $ad, 'UPDATE', 'ad_id = ' . $ad_info['ad_id'], 'SILENT');
...
function ad_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT ad_id, CASE WHEN ad_type = 0 THEN '图片'
                ...
                FROM " . $GLOBALS['xishan']->table("ad_custom") . "
                $where
 ...
 */
 function modfiy_ad_status($ad_id, $ad_status = 0)
 {
    ...
    /* 查询自定义广告信息 */
    $sql = "SELECT ad_type, content, url, ad_status FROM " . $GLOBALS['xishan']->table("ad_custom") . " WHERE ad_id = $ad_id LIMIT 0, 1";
    $ad = $GLOBALS['db']->getRow($sql);
    ...
    if ($ad_status == 1)
    {
        ...
        if ($ad['ad_status'] == 0)
        {
            $sql = "UPDATE " . $GLOBALS['xishan']->table("ad_custom") . " SET ad_status = 1 WHERE ad_id = $ad_id";
            $GLOBALS['db']->query($sql);
        }

        /* 关闭 其它自定义广告 */
        $sql = "UPDATE " . $GLOBALS['xishan']->table("ad_custom") . " SET ad_status = 0 WHERE ad_id <> $ad_id";
        $GLOBALS['db']->query($sql);

        /* 用户自定义广告开启 */
        $sql = "UPDATE " . $GLOBALS['xishan']->table("shop_config") . " SET value = 'cus' WHERE id =337";
        $GLOBALS['db']->query($sql);
    }
    else
    {
        ...
        if ($ad['ad_status'] == 0)
        {
            $sql = "SELECT COUNT(ad_id) FROM " . $GLOBALS['xishan']->table("ad_custom") . " WHERE ad_status = 1";
            $ad_status_1 = $GLOBALS['db']->getOne($sql);
            if (empty($ad_status_1))
            {
                $sql = "UPDATE " . $GLOBALS['xishan']->table("shop_config") . " SET value = 'sys' WHERE id =337";
                $GLOBALS['db']->query($sql);
            }
            else
            {
