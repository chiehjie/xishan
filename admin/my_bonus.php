 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: my_bonus.php 17217 2011-01-19 06:29:08Z liubo $
*/
...
/* 初始化$exc对象 */
$exc = new exchange($xishan->table('bonus_type'), $db, 'type_id', 'type_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('text' => $_LANG['bonustype_add'], 'href' => 'my_bonus.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('my_bonus_type.htm');
}
...
if ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('my_bonus_type.htm'), '',
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    /* 更新商品信息 */
    $db->query("UPDATE " .$xishan->table('goods'). " SET bonus_type_id = 0 WHERE bonus_type_id = '$id'");

    /* 删除用户的红包 */
    $db->query("DELETE FROM " .$xishan->table('user_bonus'). " WHERE bonus_type_id = '$id'");

    $url = 'my_bonus.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link',  array('href' => 'my_bonus.php?act=list', 'text' => $_LANG['04_bonustype_list']));
    ...
    assign_query_info();
    $smarty->display('my_bonus_type_info.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 检查类型是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$xishan->table('bonus_type'). " WHERE type_name='$type_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$xishan->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
    ...
    $link[0]['href'] = 'my_bonus.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'my_bonus.php?act=list';
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $bonus_arr = $db->getRow("SELECT * FROM " .$xishan->table('bonus_type'). " WHERE type_id = '$type_id'");
    ...
    $smarty->assign('action_link', array('href' => 'my_bonus.php?act=list&' . list_link_postfix(), 'text' => $_LANG['04_bonustype_list']));
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$xishan->table('bonus_type'). " SET ".
           "type_name       = '$type_name', ".
   ...
   /* 提示信息 */
   $link[] = array('text' => $_LANG['back_list'], 'href' => 'my_bonus.php?act=list&' . list_link_postfix());
...
if ($_REQUEST['act'] == 'send')
{
    ...
    {
        /* 查询此红包类型信息 */
        $bonus_type = $db->GetRow("SELECT type_id, type_name FROM ".$xishan->table('bonus_type').
        ...
        /* 查询其他红包类型的商品 */
        $sql = "SELECT goods_id FROM " .$xishan->table('goods').
...
if ($_REQUEST['act'] == 'send_by_user')
{
    ...
    if (isset($_REQUEST['send_rank']))
    {
        ...
        if ($rank_id > 0)
        {
            $sql = "SELECT min_points, max_points, special_rank FROM " . $xishan->table('user_rank') . " WHERE rank_id = '$rank_id'";
            $row = $db->getRow($sql);
            if ($row['special_rank'])
            {
                /* 特殊会员组处理 */
                $sql = 'SELECT COUNT(*) FROM ' . $xishan->table('users'). " WHERE user_rank = '$rank_id'";
                $send_count = $db->getOne($sql);
                if($validated_email)
                {
                    $sql = 'SELECT user_id, email, user_name FROM ' . $xishan->table('users').
            ...
            {
                $sql = 'SELECT COUNT(*) FROM ' . $xishan->table('users').
    ...
    foreach ($user_list AS $key => $val)
    {
        ...
        {
             /* 向会员红包表录入数据 */
            $sql = "INSERT INTO " . $xishan->table('user_bonus') .
    ...
    {
        /*  */
        $href = "my_bonus.php?act=send_by_user&start=" . ($start+$limit) . "&limit=$limit&id=$_REQUEST[id]&";
    ...
    $link[] = array('text' => $_LANG['back_list'], 'href' => 'my_bonus.php?act=list');
...
if ($_REQUEST['act'] == 'send_mail')
{
    ...
    $link[0]['href'] = 'my_bonus.php?act=bonus_list&bonus_type=' . $bonus['bonus_type_id'];
...
if ($_REQUEST['act'] == 'send_by_print')
{
    ...
    /* 生成红包序列号 */
    $num = $db->getOne("SELECT MAX(bonus_sn) FROM ". $xishan->table('user_bonus'));
    $num = $num ? floor($num / 10000) : 100000;

    for ($i = 0, $j = 0; $i < $bonus_sum; $i++)
    {
        $bonus_sn = ($num + $i) . str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
        $db->query("INSERT INTO ".$xishan->table('user_bonus')." (bonus_type_id, bonus_sn) VALUES('$bonus_typeid', '$bonus_sn')");
    ...
    /* 提示信息 */
    $link[0]['text'] = $_LANG['back_bonus_list'];
    $link[0]['href'] = 'my_bonus.php?act=bonus_list&bonus_type=' . $bonus_typeid;
...
if ($_REQUEST['act'] == 'gen_excel')
{
    ...
    $type_name = $db->getOne("SELECT type_name FROM ".$xishan->table('bonus_type')." WHERE type_id = '$tid'");
    ...
    $sql = "SELECT ub.bonus_id, ub.bonus_type_id, ub.bonus_sn, bt.type_name, bt.type_money, bt.use_end_date ".
           "FROM ".$xishan->table('user_bonus')." AS ub, ".$xishan->table('bonus_type')." AS bt ".
...
if ($_REQUEST['act'] == 'add_bonus_goods')
{
    ...
    {
        $sql = "UPDATE " .$xishan->table('goods'). " SET bonus_type_id='$type_id' WHERE goods_id='$val'";
...
if ($_REQUEST['act'] == 'drop_bonus_goods')
{
    ...
    $db->query("UPDATE ".$xishan->table('goods')." SET bonus_type_id = 0 ".
...
if ($_REQUEST['act'] == 'search_users')
{
    ...
    $sql = "SELECT user_id, user_name FROM " . $xishan->table('users') .
...
if ($_REQUEST['act'] == 'query_bonus')
{
    ...
    make_json_result($smarty->fetch('my_bonus_list.htm'), '',
        array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
...
elseif ($_REQUEST['act'] == 'remove_bonus')
{
    ...
    $db->query("DELETE FROM " .$xishan->table('user_bonus'). " WHERE bonus_id='$id'");

    $url = 'my_bonus.php?act=query_bonus&' . str_replace('act=remove_bonus', '', $_SERVER['QUERY_STRING']);
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        if (isset($_POST['drop']))
        {
            $sql = "DELETE FROM " . $xishan->table('user_bonus'). " WHERE bonus_id " . db_create_in($bonus_id_list);
            $db->query($sql);
            ...
            $link[] = array('text' => $_LANG['back_my_bonus_list'],
                'href' => 'my_bonus.php?act=bonus_list&bonus_type='. $bonus_type_id);
...
function get_type_list()
{
    ...
    $sql = "SELECT bonus_type_id, COUNT(*) AS sent_count".
            " FROM " .$GLOBALS['xishan']->table('user_bonus') .
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xishan']->table('bonus_type');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " .$GLOBALS['xishan']->table('bonus_type'). " ORDER BY $filter[sort_by] $filter[sort_order]";
...
{
    $sql = "SELECT goods_id, goods_name FROM " .$GLOBALS['xishan']->table('goods').
...
function get_bonus_list()
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xishan']->table('user_bonus'). $where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    ...
    $sql = "SELECT ub.*, u.user_name, u.email, o.order_sn, bt.type_name ".
          " FROM ".$GLOBALS['xishan']->table('user_bonus'). " AS ub ".
          " LEFT JOIN " .$GLOBALS['xishan']->table('bonus_type'). " AS bt ON bt.type_id=ub.bonus_type_id ".
          " LEFT JOIN " .$GLOBALS['xishan']->table('users'). " AS u ON u.user_id=ub.user_id ".
          " LEFT JOIN " .$GLOBALS['xishan']->table('order_info'). " AS o ON o.order_id=ub.order_id $where ".
...
{
    $sql = "SELECT * FROM " . $GLOBALS['xishan']->table('bonus_type') .
...
function send_bonus_mail($bonus_type_id, $bonus_id_list)
{
    ...
    $sql = "SELECT b.bonus_id, u.user_name, u.email " .
            "FROM " . $GLOBALS['xishan']->table('user_bonus') . " AS b, " .
...
function add_to_maillist($username, $email, $subject, $content, $is_html)
{
    ...
    $template_id = $GLOBALS['db']->getOne("SELECT template_id FROM " . $GLOBALS['xishan']->table('mail_templates') . " WHERE template_code = 'send_bonus'");
    $sql = "INSERT INTO "  . $GLOBALS['xishan']->table('email_sendlist') . " ( email, template_id, email_content, pri, last_send) VALUES ('$email', $template_id, '$content', 1, '$time')";
    $GLOBALS['db']->query($sql);
    return true;
}
