...
$exc = new exchange($xishan->table("category"), $db, 'cat_id', 'cat_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    //二次开发
    $smarty->assign('cat_info',  get_cat_list());
    /* 列表页面 */
    assign_query_info();
...
if ($_REQUEST['act'] == 'add')
{
    ...
    // 二次开发
    admin_priv('my_dishes_class_add');
    ...
    // //二次开发
    // $smarty->assign('select_store',  get_store_list());
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 初始化变量 */
    //二次开发
    $cat['store_id']    = !empty($_POST['store_id'])    ? intval($_POST['store_id'])  : 0;
 ...
 if (!empty($_POST['store_id']))
    {
        $sql = "SELECT * FROM  ecs_bonus_type where type_id='$_POST[store_id]'";
        $row = $db->getRow($sql);
    ...
    /* 入库的操作 */
    if ($db->autoExecute($xishan->table('category'), $cat) !== false)
    {
        $cat_id = $db->insert_id();
        if($cat['show_in_nav'] == 1)
        {
            $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $xishan->table('nav') . " WHERE type = 'middle'");
            $vieworder += 2;
            //显示在自定义导航栏中
            $sql = "INSERT INTO " . $xishan->table('nav') .
...
{
    admin_priv('my_dishes_class_add');   // 权限检查
    ...
    if ($cat_info['filter_attr'])
    {
        ...
        {
            $attr_cat_id = $db->getOne("SELECT cat_id FROM " . $xishan->table('attribute') . " WHERE attr_id = '" . intval($v) . "'");
    ...
    //分类是否存在首页推荐
    $res = $db->getAll("SELECT recommend_type FROM " . $xishan->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
    if (!empty($res))
    {
        $cat_recommend = array();
        foreach($res as $data)
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $dat = $db->getRow("SELECT cat_name, show_in_nav FROM ". $xishan->table('category') . " WHERE cat_id = '$cat_id'");

    if ($db->autoExecute($xishan->table('category'), $cat, 'UPDATE', "cat_id='$cat_id'"))
    {
        if($cat['cat_name'] != $dat['cat_name'])
        {
            //如果分类名称发生了改变
            $sql = "UPDATE " . $xishan->table('nav') . " SET name = '" . $cat['cat_name'] . "' WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
            $db->query($sql);
        }
        if($cat['show_in_nav'] != $dat['show_in_nav'])
        {
            ...
            {
                //显示
                $nid = $db->getOne("SELECT id FROM ". $xishan->table('nav') . " WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                if(empty($nid))
                {
                    //不存在
                    $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $xishan->table('nav') . " WHERE type = 'middle'");
                    ...
                    $sql = "INSERT INTO " . $xishan->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $cat['cat_name'] . "', 'c', '$cat_id','1','$vieworder','0', '" . $uri . "','middle')";
                }
                else
                {
                    $sql = "UPDATE " . $xishan->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
                }
                $db->query($sql);
            }
            else
            {
                //去除
                $db->query("UPDATE " . $xishan->table('nav') . " SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
            }
        }
...
{
    /* 权限检查 */
    admin_priv('my_dishes_class_drop');
...
if ($_REQUEST['act'] == 'move_cat')
{
    ...
    /* 更新商品分类 */
    $sql = "UPDATE " .$xishan->table('goods'). " SET cat_id = '$target_cat_id' ".
           "WHERE cat_id = '$cat_id'";
    if ($db->query($sql))
    {
        /* 清除缓存 */
...
if ($_REQUEST['act'] == 'toggle_show_in_nav')
{
    ...
    if (cat_update($id, array('show_in_nav' => $val)) != false)
    {
        if($val == 1)
        {
            ...
            $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $xishan->table('nav') . " WHERE type = 'middle'");
            $vieworder += 2;
            $catname = $db->getOne("SELECT cat_name FROM ". $xishan->table('category') . " WHERE cat_id = '$id'");
            //显示在自定义导航栏中
            $_CFG['rewrite'] = 0;
            $uri = build_uri('category', array('cid'=> $id), $catname);

            $nid = $db->getOne("SELECT id FROM ". $xishan->table('nav') . " WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
            if(empty($nid))
            {
                //不存在
                $sql = "INSERT INTO " . $xishan->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $catname . "', 'c', '$id','1','$vieworder','0', '" . $uri . "','middle')";
            }
            else
            {
                $sql = "UPDATE " . $xishan->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'";
            }
            $db->query($sql);
        }
        else
        {
            //去除
            $db->query("UPDATE " . $xishan->table('nav') . "SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
        }
        clear_cache_files();
        make_json_result($val);
    }
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $cat_name = $db->getOne('SELECT cat_name FROM ' .$xishan->table('category'). " WHERE cat_id='$cat_id'");

    /* 当前分类下是否有子分类 */
    $cat_count = $db->getOne('SELECT COUNT(*) FROM ' .$xishan->table('category'). " WHERE parent_id='$cat_id'");

    /* 当前分类下是否存在商品 */
    $goods_count = $db->getOne('SELECT COUNT(*) FROM ' .$xishan->table('goods'). " WHERE cat_id='$cat_id'");

    /* 如果不存在下级子分类和商品，则删除之 */
    if ($cat_count == 0 && $goods_count == 0)
    {
        /* 删除分类 */
        $sql = 'DELETE FROM ' .$xishan->table('category'). " WHERE cat_id = '$cat_id'";
        if ($db->query($sql))
        {
            $db->query("DELETE FROM " . $xishan->table('nav') . "WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
            clear_cache_files();
...
{
    $sql = "SELECT * FROM " .$GLOBALS['xishan']->table('category'). " WHERE cat_id='$cat_id' LIMIT 1";
...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('category'), $args, 'update', "cat_id='$cat_id'");
}
...
function get_attr_list()
{
    $sql = "SELECT a.attr_id, a.cat_id, a.attr_name ".
           ...
           $GLOBALS['xishan']->table('goods_type') . " AS c ".
...
function insert_cat_recommend($recommend_type, $cat_id)
{
    ...
    {
        //取得之前的分类
        $recommend_res = $GLOBALS['db']->getAll("SELECT recommend_type FROM " . $GLOBALS['xishan']->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
        if (empty($recommend_res))
        {
            foreach($recommend_type as $data)
            {
                $data = intval($data);
                $GLOBALS['db']->query("INSERT INTO " . $GLOBALS['xishan']->table("cat_recommend") . "(cat_id, recommend_type) VALUES ('$cat_id', '$data')");
            }
        }
        else
        {
            ...
            {
                $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['xishan']->table("cat_recommend") . " WHERE cat_id=$cat_id AND recommend_type " . db_create_in($delete_array));
            }
    ...
    {
        $GLOBALS['db']->query("DELETE FROM ". $GLOBALS['xishan']->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
    }
}
/* 获取商家列表 */
// //二次开发
