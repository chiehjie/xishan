...
/* 初始化 $exc 对象 */
$exc = new exchange($xishan->table("role"), $db, 'role_id', 'role_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$xishan->table('role')." (role_name, action_list, role_describe) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $_REQUEST['id'] = !empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
        /* 获得该管理员的权限 */
    $priv_str = $db->getOne("SELECT action_list FROM " .$xishan->table('role'). " WHERE role_id = '$_GET[id]'");
    ...
    /* 获取角色信息 */
    $sql = "SELECT role_id, role_name, role_describe FROM " .$xishan->table('role').
...
elseif ($_REQUEST['act'] == 'update')
{
    /* 更新管理员的权限 */
    $act_list = @join(",", $_POST['action_code']);
    $sql = "UPDATE " .$xishan->table('role'). " SET action_list = '$act_list', role_name = '".$_POST['user_name']."', role_describe = '".$_POST['role_describe']." ' ".
           "WHERE role_id = '$_POST[id]'";
    $db->query($sql);
    $user_sql = "UPDATE " .$xishan->table('admin_user'). " SET action_list = '$act_list' ".
           "WHERE role_id = '$_POST[id]'";
    $db->query($user_sql);
    /* 提示信息 */
...
{
    check_authz_json('admin_role_drop');

    $id = intval($_GET['id']);
    $num_sql = "SELECT count(*) FROM " .$xishan->table('admin_user'). " WHERE role_id = '$_GET[id]'";
...
function get_role_list()
{
    ...
    $sql  = 'SELECT role_id, role_name, action_list, role_describe '.
            'FROM ' .$GLOBALS['xishan']->table('role').' ORDER BY role_id DESC';
