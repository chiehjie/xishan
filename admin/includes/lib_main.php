...
function admin_log($sn = '', $action, $content)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['xishan']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
...
{
    global $xishan, $db, $_LANG;

    $sql = 'SELECT id FROM ' . $xishan->table('shop_config') . " WHERE code = '$parent' AND type = 1";
    $parent_id = $db->getOne($sql);

    $sql = 'INSERT INTO ' . $xishan->table('shop_config') . ' (parent_id, code, value) ' .
...
{
    $bonus = array();
    $sql = 'SELECT type_id, type_name, type_money FROM ' . $GLOBALS['xishan']->table('bonus_type') .
           ' WHERE send_type = 3';
...
{
    $rank_list = array();
    $sql = 'SELECT rank_id, rank_name, min_points FROM ' . $GLOBALS['xishan']->table('user_rank');
    if ($is_special)
    {
...
{
    $user_list = array();
    $sql = 'SELECT user_id, user_name FROM ' . $GLOBALS['xishan']->table('users') . $where.
...
function get_position_list()
{
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
           'FROM ' . $GLOBALS['xishan']->table('ad_position');
    $res = $GLOBALS['db']->query($sql);
...
function get_goods_list($filter)
{
    ...
    $sql = 'SELECT goods_id, goods_name, shop_price '.
           'FROM ' . $GLOBALS['xishan']->table('goods') . ' AS g ' . $where .
           'LIMIT 50';
...
function get_article_list($filter)
{
    ...
    $sql = 'SELECT a.article_id, a.title '.
           'FROM ' .$GLOBALS['xishan']->table('article'). ' AS a, ' .$GLOBALS['xishan']->table('article_cat'). ' AS c ' . $where;
    $res = $GLOBALS['db']->query($sql);
...
function area_list($region_id)
{
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['xishan']->table('region').
...
{
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['xishan']->table('goods_type') . ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
function get_pay_ids()
{
    ...
    $sql = 'SELECT pay_id, is_cod FROM ' .$GLOBALS['xishan']->table('payment'). ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = 'TRUNCATE TABLE ' .$GLOBALS['xishan']->table($table_name);
...
{
    $sql = "SELECT attr_group FROM " . $GLOBALS['xishan']->table('goods_type') . " WHERE cat_id='$cat_id'";
...
function admin_info()
{
    $sql = "SELECT * FROM ". $GLOBALS['xishan']->table('admin_user')."
