...
/* 创建 ECSHOP 对象 */
$xishan = new ECS($db_name, $prefix);
define('DATA_DIR', $xishan->data_dir());
define('IMAGE_DIR', $xishan->image_dir());
...
$sess = new cls_session($db, $xishan->table('sessions'), $xishan->table('sessions_data'), 'ECSCP_ID');
...
if(isset($_GET['ent_id']) && isset($_GET['ent_ac']) &&  isset($_GET['ent_sign']) && isset($_GET['ent_email']))
{
    ...
    if($token==md5(md5($_CFG['token']).$domain_url.ADMIN_PATH))
    {
        ...
        $db->query('UPDATE '.$xishan->table('shop_config') . ' SET value = "'. $ent_id .'" WHERE code = "ent_id"');
        $db->query('UPDATE '.$xishan->table('shop_config') . ' SET value = "'. $ent_ac .'" WHERE code = "ent_ac"');
        $db->query('UPDATE '.$xishan->table('shop_config') . ' SET value = "'. $ent_sign .'" WHERE code = "ent_sign"');
        $db->query('UPDATE '.$xishan->table('shop_config') . ' SET value = "'. $ent_email .'" WHERE code = "ent_email"');
        clear_cache_files();
...
{
    $admin_path = preg_replace('/:\d+/', '', $xishan->url()) . ADMIN_PATH;
...
if(!$certificate['certificate_id']){
    $callback = $xishan->url()."admin/certificate.php?act=get_certificate&type=index";
