...
if ($act == 'detail')
{
    ...
    if ($a_id > 0)
    {
        $article_row = $db->getRow('SELECT title, content FROM ' . $xishan->table('article') . ' WHERE article_id = ' . $a_id . ' AND cat_id > 0 AND is_open = 1');
...
{
    $article_num = $db->getOne("SELECT count(*) FROM " . $xishan->table('article') . " WHERE cat_id > 0 AND is_open = 1");
    if ($article_num > 0)
    {
        $page_num = '10';
