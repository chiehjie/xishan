...
/* 查询订单信息 */
$sql   = 'SELECT * FROM ' . $xishan->table('order_info') . " WHERE order_id = '$order_id'";
$order = $db->getRow($sql);
...
{
    /* 修改订单发货状态为“确认收货” */
    $sql = "UPDATE " . $xishan->table('order_info') . " SET shipping_status = '" . SS_RECEIVED . "',lastmodify='".time()."' WHERE order_id = '$order_id'";
    $db->query($sql);
