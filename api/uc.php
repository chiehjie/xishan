...
$ecs_url = str_replace('/api', '', $xishan->url());
...
class uc_note
{
    ...
    function renameuser($get, $post)
    {
        ...
        $this->db->query("UPDATE " . $GLOBALS['xishan']->table("users") . " SET user_name='$usernamenew' WHERE user_id='$uid'");
        $this->db->query("UPDATE " . $GLOBALS['xishan']->table("affiliate_log") . " SET user_name='$usernamenew' WHERE user_name='$usernameold'");
        $this->db->query("UPDATE " . $GLOBALS['xishan']->table("comment") . " SET user_name='$usernamenew' WHERE user_name='$usernameold'");
        $this->db->query("UPDATE " . $GLOBALS['xishan']->table("feedback") . " SET user_name='$usernamenew' WHERE user_name='$usernameold'");
        clear_cache_files();
        return API_RETURN_SUCCEED;
    }
    ...
    function updatepw($get, $post)
    {
        ...
        $this->db->query("UPDATE " . $GLOBALS['xishan']->table('users') . " SET password='$newpw' WHERE user_name='$username'");
        return API_RETURN_SUCCEED;
    }
    ...
    function updatecredit($get, $post)
    {
        ...
        $sql = "UPDATE " . $GLOBALS['xishan']-> table('users') . " SET {$points[$credit]} = {$points[$credit]} + '$amount' WHERE user_id = $uid";
        $this->db->query($sql);
        if ($this->db->affected_rows() <= 0)
        {
            return API_RETURN_FAILED;
        }
        $sql = "INSERT INTO " . $GLOBALS['xishan']->table('account_log') . "(user_id, {$points[$credit]}, change_time, change_desc, change_type)" .
    ...
    function updatecreditsettings($get, $post)
    {
        ...
        $this->db->query("UPDATE " . $GLOBALS['xishan']->table("shop_config") . " SET value='".serialize($outextcredits)."' WHERE code='points_rule'");
        return API_RETURN_SUCCEED;
    }
}
...
function delete_user($uids = '')
{
    ...
    else
    {
        ...
        $sql = "DELETE FROM " . $GLOBALS['xishan']->table('users') . " WHERE user_id IN ($uids)";
...
function set_login($user_id = '', $user_name = '')
{
    ...
    {
        $sql = "SELECT user_name, email FROM " . $GLOBALS['xishan']->table('users') . " WHERE user_id='$user_id' LIMIT 1";
        ...
        else
        {
            ...
            if($data = uc_get_user($user_name))
            {
                ...
                $sql = "REPLACE INTO " . $GLOBALS['xishan']->table('users') ."(user_id, user_name, email) VALUES('$uid', '$uname', '$email')";
...
function fetch_tag($tagname, $num=100)
{
    ...
    $sql = "SELECT t.*, u.user_name, g.goods_name, g.goods_img, g.shop_price FROM " . $GLOBALS['xishan']->table('tag') . " as t, " . $GLOBALS['xishan']->table('users') ." as u, " .
    $GLOBALS['xishan']->table('goods') ." as g WHERE tag_words = '$tagname' AND t.user_id = u.user_id AND g.goods_id = t.goods_id ORDER BY t.tag_id DESC LIMIT " . $num;
