...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id))
    {
        ...
        else
        {
            ...
            /* 上一个商品下一个商品 */
            $sql = "SELECT eg.goods_id FROM " .$xishan->table('exchange_goods'). " AS eg," . $GLOBALS['xishan']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id > " . $goods['goods_id'] . " AND eg.is_exchange = 1 AND g.is_delete = 0 LIMIT 1";
            ...
            $sql = "SELECT max(eg.goods_id) FROM " . $xishan->table('exchange_goods') . " AS eg," . $GLOBALS['xishan']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id < ".$goods['goods_id'] . " AND eg.is_exchange = 1 AND g.is_delete = 0";
...
{
    return $GLOBALS['db']->getRow('SELECT keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['xishan']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
            ...
            'FROM ' . $GLOBALS['xishan']->table('exchange_goods') . ' AS eg, ' .$GLOBALS['xishan']->table('goods') . ' AS g ' .
...
function get_exchange_goods_count($children, $min = 0, $max = 0, $ext='')
{
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['xishan']->table('exchange_goods') . ' AS eg, ' .
           $GLOBALS['xishan']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND $where $ext";
...
function get_exchange_recommend_goods($type = '', $cats = '', $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
            ...
            'FROM ' . $GLOBALS['xishan']->table('exchange_goods') . ' AS eg ' .
            'LEFT JOIN ' . $GLOBALS['xishan']->table('goods') . ' AS g ON g.goods_id = eg.goods_id ' .
...
function get_exchange_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, eg.exchange_integral, eg.is_exchange ' .
            ...
            'LEFT JOIN ' . $GLOBALS['xishan']->table('exchange_goods') . ' AS eg ON g.goods_id = eg.goods_id ' .
