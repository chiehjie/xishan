...
class integrate
{
    ...
    function integrate($cfg)
    {
        ...
        {
            $this->db_name = $GLOBALS['xishan']->db_name;
            $this->prefix = $GLOBALS['xishan']->prefix;
            $this->db = $GLOBALS['db'];
        }
        else
        {
    ...
    function edit_user($cfg)
    {
        ...
        if ((!empty($cfg['email'])) && $this->field_email != 'NULL')
        {
            ...
            {
                // 新的E-mail
                $sql = "UPDATE " . $GLOBALS['xishan']->table('users') . " SET is_validated = 0 WHERE user_name = '$cfg[post_username]'";
                $this->db->query($sql);
            }
    ...
    function sync ($username, $password='', $md5password='')
    {
        ...
        if (empty($profile))
        {
            ...
            {
               $sql = "INSERT INTO " . $GLOBALS['xishan']->table('users').
