...
class ucenter extends integrate
{
    ...
    function login($username, $password, $remember = null)
    {
        ...
        {
            //检查用户是否存在,不存在直接放入用户表
            $result = $this->db->getRow("SELECT user_id,ec_salt FROM " . $GLOBALS['xishan']->table("users") . " WHERE user_name='$username'");
            $name_exist =$result['user_id'];
            if(empty($result['ec_salt']))
            {
                $user_exist = $this->db->getOne("SELECT user_id FROM " . $GLOBALS['xishan']->table("users") . " WHERE user_name='$username' AND password = '" . MD5($password) ."'");
                if(!empty($user_exist))
                {
                    $ec_salt=rand(1,9999);
                    $this->db->query('UPDATE ' . $GLOBALS['xishan']->table("users") . "SET `password`='".MD5(MD5($password). $ec_salt)."',`ec_salt`='". $ec_salt."' WHERE user_id = '" . $uid . "'");

                }
            }
            else
            {
                $user_exist = $this->db->getOne("SELECT user_id FROM " . $GLOBALS['xishan']->table("users") . " WHERE user_name='$username' AND password = '" . MD5(MD5($password). $result['ec_salt'])."'");
            }
            ...
            if (empty($user_exist))
            {
                if(empty($name_exist))
                {
                    ...
                    $this->db->query('INSERT INTO ' . $GLOBALS['xishan']->table("users") . "(`user_id`, `email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`) VALUES ('$uid', '$email', '$uname', '$password', '$reg_date', '$reg_date', '$ip')");
                }
                else 
                {
                    if(empty($result['ec_salt']))
                    ...
                    $this->db->query('UPDATE ' . $GLOBALS['xishan']->table("users") . "SET `password`='".MD5(MD5($password). $result['ec_salt'])."',`ec_salt`='". $result['ec_salt']."' WHERE user_id = '" . $uid . "'");
                }
            }
    ...
    function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date = 0, $md5password = '')
    {
        ...
        else
        {
            ...
            $ip = real_ip();
            $password = $this->compile_password(array('password'=>$password));
            $this->db->query('INSERT INTO ' . $GLOBALS['xishan']->table("users") . "(`user_id`, `email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`) VALUES ('$uid', '$email', '$username', '$password', '$reg_date', '$reg_date', '$ip')");
            return true;
        }
    }
    ...
    function edit_user($cfg, $forget_pwd = '0')
    {
        ...
        if (!empty($set_str))
        {
            $sql = "UPDATE " . $GLOBALS['xishan']->table('users') . " SET $set_str  WHERE user_name = '$cfg[username]'";
    ...
    function get_profile_by_name($username)
    {
        ...
        $sql = "SELECT user_id, user_name, email, sex, reg_time FROM " . $GLOBALS['xishan']->table('users') . " WHERE user_name='$username'";
    ...
    function set_cookie($username='', $remember = null)
    {
        ...
        else
        {
            ...
            $sql = "SELECT user_id, password FROM " . $GLOBALS['xishan']->table('users') . " WHERE user_name='$username' LIMIT 1";
    ...
    function set_session ($username='')
    {
        ...
        {
            $sql = "SELECT user_id, password, email FROM " . $GLOBALS['xishan']->table('users') . " WHERE user_name='$username' LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
    ...
    {
        $sql = "SELECT user_id, user_name, email, sex, birthday, reg_time FROM " . $GLOBALS['xishan']->table('users') . " WHERE user_id='$id'";
        $row = $this->db->getRow($sql);
    ...
    function remove_user($id)
    {
        ...
        $sql = "SELECT user_id FROM "  . $GLOBALS['xishan']->table('users') . " WHERE ";
        $sql .= (is_array($post_id)) ? db_create_in($post_id, 'user_name') : "user_name='". $post_id . "' LIMIT 1";
        $col = $GLOBALS['db']->getCol($sql);

        if ($col)
        {
            $sql = "UPDATE " . $GLOBALS['xishan']->table('users') . " SET parent_id = 0 WHERE " . db_create_in($col, 'parent_id'); //将删除用户的下级的parent_id 改为0
            $GLOBALS['db']->query($sql);
            $sql = "DELETE FROM " . $GLOBALS['xishan']->table('users') . " WHERE " . db_create_in($col, 'user_id'); //删除用户
            $GLOBALS['db']->query($sql);
            /* 删除用户订单 */
            $sql = "SELECT order_id FROM " . $GLOBALS['xishan']->table('order_info') . " WHERE " . db_create_in($col, 'user_id');
            $GLOBALS['db']->query($sql);
            $col_order_id = $GLOBALS['db']->getCol($sql);
            if ($col_order_id)
            {
                $sql = "DELETE FROM " . $GLOBALS['xishan']->table('order_info') . " WHERE " . db_create_in($col_order_id, 'order_id');
                $GLOBALS['db']->query($sql);
                $sql = "DELETE FROM " . $GLOBALS['xishan']->table('order_goods') . " WHERE " . db_create_in($col_order_id, 'order_id');
                $GLOBALS['db']->query($sql);
            }

            $sql = "DELETE FROM " . $GLOBALS['xishan']->table('booking_goods') . " WHERE " . db_create_in($col, 'user_id'); //删除用户
            $GLOBALS['db']->query($sql);
            $sql = "DELETE FROM " . $GLOBALS['xishan']->table('collect_goods') . " WHERE " . db_create_in($col, 'user_id'); //删除会员收藏商品
            $GLOBALS['db']->query($sql);
            $sql = "DELETE FROM " . $GLOBALS['xishan']->table('feedback') . " WHERE " . db_create_in($col, 'user_id'); //删除用户留言
            $GLOBALS['db']->query($sql);
            $sql = "DELETE FROM " . $GLOBALS['xishan']->table('user_address') . " WHERE " . db_create_in($col, 'user_id'); //删除用户地址
            $GLOBALS['db']->query($sql);
            $sql = "DELETE FROM " . $GLOBALS['xishan']->table('user_bonus') . " WHERE " . db_create_in($col, 'user_id'); //删除用户红包
            $GLOBALS['db']->query($sql);
            $sql = "DELETE FROM " . $GLOBALS['xishan']->table('user_account') . " WHERE " . db_create_in($col, 'user_id'); //删除用户帐号金额
            $GLOBALS['db']->query($sql);
            $sql = "DELETE FROM " . $GLOBALS['xishan']->table('tag') . " WHERE " . db_create_in($col, 'user_id'); //删除用户标记
            $GLOBALS['db']->query($sql);
            $sql = "DELETE FROM " . $GLOBALS['xishan']->table('account_log') . " WHERE " . db_create_in($col, 'user_id'); //删除用户日志
            $GLOBALS['db']->query($sql);
        }
        ...
        $sql = "DELETE FROM " . $GLOBALS['xishan']->table('users') . " WHERE ";
        if (is_array($post_id))
        {
