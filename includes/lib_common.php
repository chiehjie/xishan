...
{
    $sql = 'SELECT region_id, region_name FROM ' . $GLOBALS['xishan']->table('region') .
...
{
    /* 获得配置信息 */
    $sql = 'SELECT configure FROM ' . $GLOBALS['xishan']->table('shipping_area') . " WHERE shipping_area_id = '$area_id'";
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            ...
            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    " FROM " . $GLOBALS['xishan']->table('goods_cat') . " AS gc , " . $GLOBALS['xishan']->table('goods') . " AS g " .
...
function load_config()
{
    ...
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['xishan']->table('shop_config') . ' WHERE parent_id > 0';
        $res = $GLOBALS['db']->getAll($sql);
...
{
    $sql = 'SELECT brand_id, brand_name FROM ' . $GLOBALS['xishan']->table('brand') . ' ORDER BY sort_order';
    $res = $GLOBALS['db']->getAll($sql);
...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
            "FROM " . $GLOBALS['xishan']->table('brand') . "AS b, ".
...
function get_promotion_info($goods_id = '')
{
    ...
    $gmtime = gmtime();
    $sql = 'SELECT act_id, act_name, act_type, start_time, end_time FROM ' . $GLOBALS['xishan']->table('goods_activity') . " WHERE is_finished=0 AND start_time <= '$gmtime' AND end_time >= '$gmtime'";
    if(!empty($goods_id))
    {
    ...
    $favourable = array();
    $sql = 'SELECT act_id, act_range, act_range_ext, act_name, start_time, end_time FROM ' . $GLOBALS['xishan']->table('favourable_activity') . " WHERE start_time <= '$gmtime' AND end_time >= '$gmtime'";
    if(!empty($goods_id))
    {
    ...
    {
        $sql = "SELECT cat_id, brand_id FROM " . $GLOBALS['xishan']->table('goods') .
...
{
    $sql = 'SELECT template_subject, is_html, template_content FROM ' . $GLOBALS['xishan']->table('mail_templates') . " WHERE template_code = '$tpl_name'";
...
function order_action($order_sn, $order_status, $shipping_status, $pay_status, $note = '', $username = null, $place = 0)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['xishan']->table('order_action') .
            ...
            'FROM ' . $GLOBALS['xishan']->table('order_info') . " WHERE order_sn = '$order_sn'";
...
function get_virtual_goods($order_id, $shipping = false)
{
    if ($shipping)
    {
        $sql = 'SELECT goods_id, goods_name, send_number AS num, extension_code FROM '.
           $GLOBALS['xishan']->table('order_goods') .
...
function virtual_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    ...
    /* 检查有没有缺货 */
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['xishan']->table('virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0 ";
    ...
    if ($num < $goods['num'])
    {
     ...
     /* 取出卡片信息 */
     $sql = "SELECT card_id, card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['xishan']->table('virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0  LIMIT " . $goods['num'];
    ...
    /* 更新库存 */
    $sql = "UPDATE ".$GLOBALS['xishan']->table('goods'). " SET goods_number = goods_number - '$goods[num]' WHERE goods_id = '$goods[goods_id]'";
    $GLOBALS['db']->query($sql);
...
function virtual_card_result($order_sn, $goods)
{
    ...
    /* 获取已经发送的卡片数据 */
    $sql = "SELECT card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['xishan']->table('virtual_card')." WHERE goods_id= '$goods[goods_id]' AND order_sn = '$order_sn' ";
...
function get_snatch_result($id)
{
    $sql = 'SELECT u.user_id, u.user_name, u.email, lg.bid_price, lg.bid_time, count(*) as num' .
            ' FROM ' . $GLOBALS['xishan']->table('snatch_log') . ' AS lg '.
            ' LEFT JOIN ' . $GLOBALS['xishan']->table('users') . ' AS u ON lg.user_id = u.user_id'.
    ...
    if ($rec)
    {
        ...
        /* 活动信息 */
        $sql = 'SELECT ext_info " .
               " FROM ' . $GLOBALS['xishan']->table('goods_activity') .
...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('account_log'), $account_log, 'INSERT');

    /* 更新用户信息 */
    $sql = "UPDATE " . $GLOBALS['xishan']->table('users') .
...
function log_account_other_change($user_id, $order_id, $order_sn, $money = 0, $pay_type = '', $pay_time = '', $change_desc = '')
{
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('account_other_log'), $account_other_log, 'INSERT');
}
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
               ' FROM ' . $GLOBALS['xishan']->table('article_cat') . " AS c".
               " LEFT JOIN " . $GLOBALS['xishan']->table('article_cat') . " AS s ON s.parent_id=c.cat_id".
               " LEFT JOIN " . $GLOBALS['xishan']->table('article') . " AS a ON a.cat_id=c.cat_id".
               " GROUP BY c.cat_id ".
...
function get_volume_price_list($goods_id, $price_type = '1')
{
    ...
    $sql = "SELECT `volume_number` , `volume_price`".
           " FROM " .$GLOBALS['xishan']->table('volume_price'). "".
...
function get_package_info($id)
{
    ...
    $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, ".
           ...
           "   LEFT JOIN ". $GLOBALS['xishan']->table('goods') . " AS g ".
...
function get_package_goods($package_id)
{
    ...
    if ($good_product_str != '')
    {
        $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['xishan']->table('goods_attr'). " WHERE goods_id IN ($good_product_str)";
...
function get_good_products($goods_id, $conditions = '')
{
    ...
    /* 取货品 */
    $sql = "SELECT * FROM " .$GLOBALS['xishan']->table('products'). " WHERE $_goods_id $conditions";
    $result_products = $GLOBALS['db']->getAll($sql);

    /* 取商品属性 */
    $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['xishan']->table('goods_attr'). " WHERE $_goods_id";
    $result_goods_attr = $GLOBALS['db']->getAll($sql);
...
function delete_yunqi_code(){
    $sql = "select value from ".$GLOBALS['xishan']->table('shop_config')." where code='certificate'";
    $row = $GLOBALS['db']->getOne($sql);
    if(!$row) return false;
    $certificate = unserialize($row);
    unset($certificate['yunqi_code']);
    $update_sql = "update ".$GLOBALS['xishan']->table('shop_config')." set value='".serialize($certificate)."' where code='certificate'";
...
function get_certificate_info($key,$code='certificate'){
    $sql = "select value from ".$GLOBALS['xishan']->table('shop_config')." where code='".$code."'";
    $row = $GLOBALS['db']->getOne($sql);
    if(!$row) return false;
    $certificate = unserialize($row);
...
function iframe_source_encode($product)
{
    ...
    $source[] = time();
    $source[] = urlencode($GLOBALS['xishan']->url());
