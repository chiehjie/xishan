...
function register($username, $password, $email, $other = array())
{
    ...
    else
    {
        ...
        if (isset($affiliate['on']) && $affiliate['on'] == 1)
        {
            ...
            if ($up_uid)
            {
                if (!empty($affiliate['config']['level_register_all']))
                {
                    ...
                    {
                        $rank_points = $GLOBALS['db']->getOne("SELECT rank_points FROM " . $GLOBALS['xishan']->table('users') . " WHERE user_id = '$up_uid'");
                ...
                //设置推荐人
                $sql = 'UPDATE '. $GLOBALS['xishan']->table('users') . ' SET parent_id = ' . $up_uid . ' WHERE user_id = ' . $_SESSION['user_id'];
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('users'), $update_data, 'UPDATE', 'user_id = ' . $_SESSION['user_id']);
...
function send_pwd_email($uid, $user_name, $email, $code)
{
    ...
    $reset_email = $GLOBALS['xishan']->url() . 'user.php?act=get_password&uid=' . $uid . '&code=' . $code;
...
function send_regiter_hash ($user_id)
{
    ...
    $validate_email = $GLOBALS['xishan']->url() . 'user.php?act=validate_email&hash=' . $hash;

    $sql = "SELECT user_name, email FROM " . $GLOBALS['xishan']->table('users') . " WHERE user_id = '$user_id'";
...
{
    $res = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['xishan']->table('admin_user') .
...
{
    $sql = "select passport_uid from ".$GLOBALS['xishan']->table('admin_user')." where passport_uid != '' ";
    if($GLOBALS['db']->getOne($sql)) return false;
    $nav_list = $GLOBALS['db']->getOne("select nav_list from ".$GLOBALS['xishan']->table('admin_user')." where action_list = 'all' ");
    $sql = "insert into ".$GLOBALS['xishan']->table('admin_user')."(user_name, email, password, add_time, action_list, nav_list, passport_uid,yq_create_time)values('".$passport_uid."','".$passport_uid."','shopex',".time().",'all','".$nav_list."','".$passport_uid."',".time().")";
...
function tryLogin($data,&$error_msg){
    ...
    $sql="SELECT `ec_salt` FROM ". $GLOBALS['xishan']->table('admin_user') ."WHERE user_name = '" . $data['username']."'";
