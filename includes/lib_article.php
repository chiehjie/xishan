...
function get_article_count($cat_id ,$requirement='')
{
    ...
    if ($requirement != '')
    {
        $count = $db->getOne('SELECT COUNT(*) FROM ' . $xishan->table('article') . ' WHERE ' . get_article_children($cat_id) . ' AND  title like \'%' . $requirement . '%\'  AND is_open = 1');
    }
    else
    {
        $count = $db->getOne("SELECT COUNT(*) FROM " . $xishan->table('article') . " WHERE " . get_article_children($cat_id) . " AND is_open = 1");
    }
    return $count;
}
