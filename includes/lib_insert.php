...
function insert_query_info()
{
    ...
    $online_count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['xishan']->table('sessions'));
...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price FROM ' . $GLOBALS['xishan']->table('goods') .
...
function insert_ads($arr)
{
    ...
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                ...
                'FROM ' . $GLOBALS['xishan']->table('ad') . ' AS a '.
                'LEFT JOIN ' . $GLOBALS['xishan']->table('ad_position') . ' AS p ON a.position_id = p.position_id ' .
...
function insert_bought_notes($arr)
{
    ...
    $sql = 'SELECT u.user_name, og.goods_number, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
           'FROM ' . $GLOBALS['xishan']->table('order_info') . ' AS oi LEFT JOIN ' . $GLOBALS['xishan']->table('users') . ' AS u ON oi.user_id = u.user_id, ' . $GLOBALS['xishan']->table('order_goods') . ' AS og ' .
