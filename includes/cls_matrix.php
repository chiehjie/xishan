...
class matrix
{
    ...
    function get_callback($type_id){
        $sql = "select * from ".$GLOBALS['xishan']->table('callback_status')." where type_id='".$type_id."' and status='false'";
    ...
    function get_card_info($order_sn,$card){
        $gift_value = array();
        $sql = "SELECT * FROM ".$GLOBALS['xishan']->table('card')." WHERE card_name = '{$card['card_name']}'";
    ...
    function get_order_goods($res,$has_product){
        ...
        foreach ($res as $key => $value) {
            ...
            //     $sql = "SELECT sum(attr_price) as attr_price FROM ".$GLOBALS['xishan']->table('goods_attr')." WHERE goods_id = '".$value['goods_id']."' AND goods_attr_id in (".str_replace("|", ",", $value['p_goods_attr']).")";
    ...
    function getGoods($order_id,$order_sn,$card=false,$use_gift=false,$card_fee=0){
        ...
        $sql = "SELECT og.*,p.product_sn,g.goods_weight,g.give_integral,g.shop_price,p.goods_attr as p_goods_attr FROM ".$GLOBALS['xishan']->table('order_goods')." as og LEFT JOIN ".$GLOBALS['xishan']->table('products')." as p on og.product_id = p.product_id LEFT JOIN ".$GLOBALS['xishan']->table('goods')." as g on p.goods_id = g.goods_id WHERE og.order_id = {$order_id} AND og.product_id>0";
        $res = $GLOBALS['db']->getAll($sql);
        if ($res) {
            $return_has_pro = $this->get_order_goods($res,$has_product=true);
        }
        $sql = "SELECT og.*,g.goods_weight,g.give_integral,g.shop_price FROM ".$GLOBALS['xishan']->table('order_goods')." as og LEFT JOIN ".$GLOBALS['xishan']->table('goods')." as g on og.goods_id = g.goods_id WHERE og.order_id = {$order_id} AND og.product_id=0";
        $res = $GLOBALS['db']->getAll($sql);
        if ($res) {
    ...
    function getOrderStruct($order_sn,$fields='*',$is_create=FALSE){
        ...
        if ($order_info['pay_status'] == '0' && $order_info['pay_time']>0) {
            $sql = "select count(user_money) as user_money from ".$GLOBALS['xishan']->table('account_log')." where user_id = '".$order_info[
        ...
        $refund_money = 0;
        $sql = "select action_note from ".$GLOBALS['xishan']->table('order_action')." where order_id = ".$order_info['order_id']." and order_status = 4 ";
        ...
        $sql = "select back_id from ".$GLOBALS['xishan']->table('back_order')." where order_id = ".$order_info['order_id']." AND order_sn='".$order_info['order_sn']."'";
    ...
    function get_payment_list($order_info){
        $payment_list = array();
        $sql = "SELECT * FROM ".$GLOBALS['xishan']->table('account_log')." WHERE change_desc like '%".$order_info['order_sn']."%' AND user_money<0 order by change_time asc";
        $res = $GLOBALS['db']->getAll($sql);
        if ($res) {
        ...
        $sql = "SELECT * FROM ".$GLOBALS['xishan']->table('account_other_log')." WHERE order_sn='".$order_info['order_sn']."'";
    ...
    function get_region($region_id,$region_type){
        $msg = '';
        $sql = "select region_name FROM ".$GLOBALS['xishan']->table('region')." WHERE region_id = {$region_id} AND region_type = {$region_type}";
