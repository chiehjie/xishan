...
class certificate
{
    ...
    {
        // 取出网店 certificate
        $sql = "select code,value from ".$GLOBALS['xishan']->table('shop_config')." where code='certificate'";
    ...
    */
    function set_shop_certificate($data){
        ...
        if($row){
            $sql = "UPDATE ".$GLOBALS['xishan']->table('shop_config')." set value='".serialize($certificate)."' where code='certificate'";
        }else{
            $sql = "insert into ".$GLOBALS['xishan']->table('shop_config')." set parent_id=2,code='certificate',type='hidden',value='".serialize($certificate)."'";
        }
    ...
    */
    function set_yunqi_account($data){
        $data['status'] = true;
        $sql = "insert into ".$GLOBALS['xishan']->table('shop_config')." set parent_id=2,code='yunqi_account',type='hidden',value='".serialize($data)."'";
    ...
    */
    function get_yunqi_account(){
        $sql = "select value from ".$GLOBALS['xishan']->table('shop_config')." where code='yunqi_account'";
    ...
    */
    function delete_cert(&$msg){
        $sql = "select passport_uid from ".$GLOBALS['xishan']->table('admin_user')." where passport_uid is not null or passport_uid!=''";
        $passport_uid = $GLOBALS['db']->getOne($sql);
        if(!$passport_uid || $passport_uid!=$_SESSION['admin_name']){
            $msg = '请使用激活的云起账号登录后再进行删除操作';
            return false;
        }
        $delete_sql = "delete from ".$GLOBALS['xishan']->table('shop_config')." where code in ('certificate','snlist','yunqi_account')";
        $user_delete = "delete from ".$GLOBALS['xishan']->table('admin_user')." where user_name='".$passport_uid."'";
     ...
     */
     function check_oauth_certificate($token){
        ...
        $params['certi_url'] = $GLOBALS['xishan']->url();
        $params['certi_session'] = STORE_KEY;
    ...
    */
    ...
    function logout_url($callback=''){
        !$callback and $callback = $GLOBALS['xishan']->url()."admin/privilege.php?act=logout&type=yunqi";
    ...
    */
    function save_snlist($data){
        ...
        $sql = "insert into ".$GLOBALS['xishan']->table('shop_config')." set parent_id=2,code='snlist',type='hidden',value='".json_encode($_data)."'";
        $GLOBALS['db']->query($sql,SILENT);
    }
    ...
    function shop_config($code){
        ...
        $sql = "select code,value from ".$GLOBALS['xishan']->table('shop_config').$where;
        if(is_array($code)){
    ...
    */
    function is_open_sn($goods_name){
        $sql = "select `value` from ".$GLOBALS['xishan']->table('shop_config')." where code='snlist'";
        $row = $GLOBALS['db']->getRow($sql);
        if(empty($row)) return false;
        $snlist = json_decode($row['value'],1);

        $sql = "select `value` from ".$GLOBALS['xishan']->table('shop_config')." where code='snlist_code'";
        $row = $GLOBALS['db']->getRow($sql);
        if(empty($row)) return false;
    ...
    function is_bind_sn($name,$type='bind_type'){
        $sql = "select `value` from ".$GLOBALS['xishan']->table('shop_config')." where code='bind_list'";
        $row = $GLOBALS['db']->getRow($sql);
        if(empty($row)) return false;
    ...
    function install_yqpayment(){
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['xishan']->table('payment') . " WHERE pay_code = 'yunqi'";

        if ($GLOBALS['db']->GetOne($sql) > 0)
        {
            /* 该支付方式已经安装过, 将该支付方式的状态设置为 enable */
            $sql = "UPDATE " . $GLOBALS['xishan']->table('payment') .
        ...
        else
        {
            ...
            $sql = "INSERT INTO " . $GLOBALS['xishan']->table('payment') . " (pay_code, pay_name, pay_desc, pay_config, is_cod, pay_fee, enabled, is_online)" .
    ...
    */
    function yqexp_exp_get($data){
        ...
        if($response['status'] == 'error' and $response['code'] == '400004'){//token失效,删除token
            delete_yunqi_code();
            $sql = "update ".$GLOBALS['xishan']->table('shop_config')." set value='0' where code='logistics_trace'";
            $GLOBALS['db']->query($sql);
        }
    ...
    */
    function yqexp_exp_active(){
        ...
        $params['siteurl'] = $GLOBALS['xishan']->url();
        $rall = $this->oauth->request($token)->post($type,$params,$time);
        $response = $rall->parsed();
        return $response;
    }
    ...
    */
    ...
    function get_push_count($type){
        $sql = "select * from ".$GLOBALS['xishan']->table('shop_config')." where code='".$type."'";
        $row = $GLOBALS['db']->getRow($sql);
        if($row){
            return $row['value'];
        }else{
    ...
    function crm_get_count($type){
        if($type == 'order'){
            ...
            $sql = "select count(*) from ". $GLOBALS['xishan']->table('users');
        }
        $row = $GLOBALS['db']->getRow($sql);
        return $row['count(*)'];
    }
