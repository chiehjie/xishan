...
function update_user_info()
{
    ...
    $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, '.
            ...
            ' FROM ' .$GLOBALS['xishan']->table('users'). ' AS u ' .
            ' LEFT JOIN ' .$GLOBALS['xishan']->table('user_bonus'). ' AS ub'.
            ' ON ub.user_id = u.user_id AND ub.used_time = 0 ' .
            ' LEFT JOIN ' .$GLOBALS['xishan']->table('bonus_type'). ' AS b'.
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if($row['user_rank'] >0)
        {
            $sql="SELECT special_rank from ".$GLOBALS['xishan']->table('user_rank')."where rank_id='$row[user_rank]'";
            if($GLOBALS['db']->getOne($sql)==='0' || $GLOBALS['db']->getOne($sql)===null)
            {   
                $sql="update ".$GLOBALS['xishan']->table('users')."set user_rank='0' where user_id='$_SESSION[user_id]'";
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['xishan']->table('user_rank') . " WHERE special_rank = '0' AND min_points <= " . intval($row['rank_points']) . ' AND max_points > ' . intval($row['rank_points']);
            if ($row = $GLOBALS['db']->getRow($sql))
            {
        ...
        {
            // 特殊等级
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['xishan']->table('user_rank') . " WHERE rank_id = '$row[user_rank]'";
            if ($row = $GLOBALS['db']->getRow($sql))
            {
                $_SESSION['user_rank'] = $row['rank_id'];
                $_SESSION['discount']  = $row['discount'] / 100.00;
    ...
    /* 更新登录时间，登录次数及登录ip */
    $sql = "UPDATE " .$GLOBALS['xishan']->table('users'). " SET".
...
function get_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['xishan']->table('category'));
...
{
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['xishan']->table('template') .
...
{
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['xishan']->table('article_cat') . " WHERE cat_id = '" . $id ."'";
...
function get_shop_help()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            'FROM ' .$GLOBALS['xishan']->table('article'). ' AS a ' .
            'LEFT JOIN ' .$GLOBALS['xishan']->table('article_cat'). ' AS c ' .
...
function get_vote($id = '')
{
    ...
    if (empty($id))
    {
        ...
        $sql = 'SELECT vote_id, vote_name, can_multi, vote_count, RAND() AS rnd' .
               ' FROM ' . $GLOBALS['xishan']->table('vote') .
    ...
    if ($vote_arr !== false && !empty($vote_arr))
    {
        ...
        $sql_option = 'SELECT v.*, o.option_id, o.vote_id, o.option_name, o.option_count ' .
                      'FROM ' . $GLOBALS['xishan']->table('vote') . ' AS v, ' .
                            $GLOBALS['xishan']->table('vote_option') . ' AS o ' .
        ...
        /* 总票数 */
        $sql = 'SELECT SUM(option_count) AS all_option FROM ' . $GLOBALS['xishan']->table('vote_option') .
...
function is_spider($record = true)
{
    ...
    foreach ($searchengine_bot AS $key => $value)
    {
        if (strpos($spider, $value) !== false)
        {
            ...
            if ($record === true)
            {
                $GLOBALS['db']->autoReplace($GLOBALS['xishan']->table('searchengine'), array('date' => local_date('Y-m-d'), 'searchengine' => $spider, 'count' => 1), array('count' => 1));
            }
...
function visit_stats()
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['xishan']->table('stats') . ' ( ' .
...
function save_searchengine_keyword($domain, $path)
{
    ...
    if (!empty($keywords))
    {
        ...
        $GLOBALS['db']->autoReplace($GLOBALS['xishan']->table('keywords'), array('date' => local_date('Y-m-d'), 'searchengine' => $searchengine, 'keyword' => htmlspecialchars(addslashes($keywords)), 'count' => 1), array('count' => 1));
    }
}
...
function get_dyna_libs($theme, $tmp)
{
    ...
    $sql = 'SELECT region, library, sort_order, id, number, type' .
            ' FROM ' . $GLOBALS['xishan']->table('template') .
...
function recalculate_price()
{
    ...
    foreach ($res AS $row)
    {
        ...
        $goods_sql = "UPDATE " .$GLOBALS['xishan']->table('cart'). " SET goods_price = '$goods_price' ".
    ...
    /* 删除赠品，重新选择 */
    $GLOBALS['db']->query('DELETE FROM ' . $GLOBALS['xishan']->table('cart') .
...
{
    /* 取得评论列表 */
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['xishan']->table('comment').
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['xishan']->table('comment') .
            " WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0".
...
function get_user_bonus($user_id = 0)
{
    ...
    $sql = "SELECT SUM(bt.type_money) AS bonus_value, COUNT(*) AS bonus_count ".
            "FROM " .$GLOBALS['xishan']->table('user_bonus'). " AS ub, ".
                $GLOBALS['xishan']->table('bonus_type') . " AS bt ".
...
function get_affiliate()
{
    if (!empty($_COOKIE['ecshop_affiliate_uid']))
    {
        ...
        if ($GLOBALS['db']->getOne('SELECT user_id FROM ' . $GLOBALS['xishan']->table('users') . "WHERE user_id = '$uid'"))
        {
            return $uid;
        }
        else
...
function article_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['xishan']->table('article_cat') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['xishan']->table('article_cat') . " WHERE parent_id = '$parent_id'";
    if ($GLOBALS['db']->getOne($sql))
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                    'b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order AS child_order ' .
                'FROM ' . $GLOBALS['xishan']->table('article_cat') . ' AS a ' .
                'LEFT JOIN ' . $GLOBALS['xishan']->table('article_cat') . ' AS b ON b.parent_id = a.cat_id ' .
...
function get_article_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['xishan']->table('article_cat'));
...
function get_library_number($library, $template = null)
{
    ...
    if (!isset($lib_list[$template]))
    {
        ...
        $sql = "SELECT library, number FROM " . $GLOBALS['xishan']->table('template') .
...
{
    $sql = 'SELECT * FROM '. $GLOBALS['xishan']->table('nav') . '
