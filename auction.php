...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    //更新商品点击次数
    $sql = 'UPDATE ' . $xishan->table('goods') . ' SET click_count = click_count + 1 '.
...
elseif ($_REQUEST['act'] == 'bid')
{
    ...
    $db->autoExecute($xishan->table('auction_log'), $auction_log, 'INSERT');

    /* 出价是否等于一口价 */
    if ($bid_price == $auction['end_price'])
    {
        /* 结束拍卖活动 */
        $sql = "UPDATE " . $xishan->table('goods_activity') . " SET is_finished = 1 WHERE act_id = '$id' LIMIT 1";
        $db->query($sql);
    }
...
function auction_list($size, $page)
{
    ...
    $sql = "SELECT a.*, IFNULL(g.goods_thumb, '') AS goods_thumb " .
            "FROM " . $GLOBALS['xishan']->table('goods_activity') . " AS a " .
                "LEFT JOIN " . $GLOBALS['xishan']->table('goods') . " AS g ON a.goods_id = g.goods_id " .
