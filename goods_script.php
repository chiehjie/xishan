...
if (!$smarty->is_cached($tpl, $cache_id))
{
    ...
    if ($type == '')
    {
        ...
        $goods_url = $xishan->url() . 'affiche.php?ad_id=-1&amp;from=' . $_from . '&amp;goods_id=';
    ...
    {
        $uid = (int)$_GET['u'];
        $goods_url = $xishan->url() . "goods.php?u=$uid&id=";
        $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.goods_thumb, IF(g.is_promote = 1 AND '$time' >= g.promote_start_date AND ".
           "'$time' <= g.promote_end_date, g.promote_price, g.shop_price) AS goods_price FROM " . $xishan->table('goods') . " g LEFT JOIN " . $xishan->table('collect_goods') . " c ON g.goods_id = c.goods_id " .
               " WHERE c.user_id = '$uid'";
    }
