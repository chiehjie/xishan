...
function ome_update_order_item(){
    ...
    if($data['orders']){
        ...
        if( in_array($order['ship_status'],array('0','1','3','4','5','6'))){
            $rs = $GLOBALS['db']->getAll("SELECT * FROM ".$GLOBALS['xishan']->table('order_goods')." WHERE order_id='".$order_id."'");
            $number_sum = 0;
            $number_num = 0;
            foreach($rs as $item){
    ...
    // 检测贺卡
    $card_info = $GLOBALS['db']->getRow("SELECT o.card_id,o.card_fee,c.card_name FROM ".$GLOBALS['xishan']->table('order_info')." as o LEFT JOIN ".$GLOBALS['xishan']->table('card')." as c on o.card_id=c.card_id WHERE o.order_id=".$order['order_id']);
    $order['card_id'] = $card_info['card_id'];
    $order['card_fee'] = $card_info['card_fee'];
    $order['card_name'] = $card_info['card_name'];

    if ($GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('order_info'),
...
function verify_goods_valid($order_sn,$data,&$msg){
    ...
    $is_goods = $GLOBALS['db']->getAll("SELECT goods_sn,is_on_sale FROM ".$GLOBALS['xishan']->table('goods')." WHERE goods_sn in ('".implode("','", $bns)."') " );
    ...
    if ($bns) {
        $is_products = $GLOBALS['db']->getAll("SELECT g.goods_sn,g.is_on_sale,p.product_sn FROM ".$GLOBALS['xishan']->table('goods')." as g right join ".$GLOBALS['xishan']->table('products')." as p on g.goods_id=p.goods_id WHERE p.product_sn in ('".implode("','", $bns)."')" );
...
function _omeUpdateOrderItem($order_sn,$data,$order){
    ...
    // 已经存在的 Item
    $rs = $db->getAll("SELECT og.*,p.product_sn FROM ".$xishan->table('order_goods')." as og left JOIN ".$xishan->table('products')." as p ON og.product_id = p.product_id WHERE order_id='".$order['order_id']."'");
    foreach($rs as $item){
        if ($item['product_id'] == 0) {
            ...
            if(in_array($item['goods_sn'],$nbns)){
                ...
                if(!$skip){
                    $db->autoExecute($GLOBALS['xishan']->table('order_goods'),$new_order_items[$item['goods_sn']], 'UPDATE', "order_id = '".$order['order_id']."' AND goods_sn = '".$item['goods_sn']."'");
                }
                unset($new_order_items[$item['goods_sn']]);
            }else{
                $i=$db->query("Delete from ".$xishan->table('order_goods')." WHERE order_id='".$order['order_id']."' AND goods_sn = '".$item['goods_sn']."'");
            }
        }else{
            // 多规格商品
            ...
            if(in_array($item['product_sn'],$nbns)){
                ...
                if(!$skip){
                    $db->autoExecute($GLOBALS['xishan']->table('order_goods'),$new_order_items[$item['product_sn']], 'UPDATE', "order_id = '".$order['order_id']."' AND product_id = '".$item['product_id']."'");
                }
                unset($new_order_items[$item['product_sn']]);
            }else{
                $i=$db->query("Delete from ".$xishan->table('order_goods')." WHERE order_id='".$order['order_id']."' AND product_id = '".$item['product_id']."'");
            }
        }
    }
    $fail_order_items = '';
    ...
    foreach($new_order_items as $key=>$item){
        ...
        $goodId=$r['goods_id'];
        if(!$goodId) continue;
        $sql = "insert into ".$GLOBALS['xishan']->table('order_goods')." set order_id={$order['order_id']},goods_id={$r['goods_id']},goods_name='{$r['goods_name']}',goods_sn='{$r['goods_sn']}',product_id={$r['product_id']},goods_number={$item['goods_number']},market_price='{$r['market_price']}',goods_price='{$item['goods_price']}',discount_fee='{$item['discount_fee']}',goods_attr='{$r['goods_attr']}',send_number={$item['send_number']},is_real='{$r['is_real']}',is_gift='{$item['is_gift']}',goods_attr_id='{$r['goods_attr_id']}' ";
        $aRs = $db->query($sql);
    }
...
function getFieldById($id, $aFeild=array('*')){
    $sqlString = "SELECT ".implode(',', $aFeild)." FROM ".$GLOBALS['xishan']->table('products')." WHERE product_id = ".intval($id);
...
function get_product($bn,$order){
    if (strpos($bn, 'ECS_CARD_') !== FALSE) {
        ...
        if ($card_id) {
            ...
            $sql = "update ".$GLOBALS['xishan']->table('order_info')." set card_id = {$card_id},card_fee = '{$card_fee}' WHERE order_id = '{$order['order_id']}'";
    ...
    $db = $GLOBALS['db'];
    $xishan = $GLOBALS['xishan'];
    if($tmp = $db->getRow("SELECT goods_id,goods_name,goods_sn,market_price,is_real,is_on_sale FROM ".$xishan->table('goods')." WHERE goods_sn='".$bn."'")){
        //商品
    ...
    }else if($aData = $db->getRow("SELECT g.goods_id,g.goods_name,g.goods_sn,g.market_price,g.is_real,g.is_on_sale,p.product_id,p.goods_attr as goods_attr_id FROM ".$xishan->table('goods')." as g right join ".$xishan->table('products')." as p on g.goods_id = p.goods_id WHERE p.product_sn='".$bn."'")){
        //货品
        if($aData['is_on_sale'] == '0'){
            ...
            $aData['goods_attr'] = '';
            $attr_info = $db->getAll("SELECT ga.attr_value,a.attr_name FROM ".$xishan->table('goods_attr')." as ga LEFT JOIN ".$xishan->table('attribute')." as a on ga.attr_id=a.attr_id WHERE ga.goods_attr_id in ({$aData['goods_attr_id']})");
            if ($attr_info) {
...
function getDlTypeList(){
    $sql = "SELECT shipping_id,shipping_name from ".$GLOBALS['xishan']->table('shipping')." WHERE enabled = 1";
...
function search_order_lists(){
    ...
    //搜索结果
    $sql_count = 'select count(*) as total_results from '.$GLOBALS['xishan']->table('order_info').' where '.implode(' and ', $where).' ';
    $result = $GLOBALS['db']->getRow($sql_count);
    if(!$result['total_results']) data_back('true','',RETURN_TYPE);

    $sql = 'select order_sn from '.$GLOBALS['xishan']->table('order_info').' where '.implode(' and ', $where).' order by lastmodify limit '.($page_no-1)*$page_size.','.$page_size;
...
function update_consignee(){
    ...
    $sql = "SELECT * FROM " . $GLOBALS['xishan']->table('order_info') .
    ...
    $sql="SELECT region_id FROM ".$GLOBALS['xishan']->table('region')." WHERE region_type = '1' AND region_name = '".$state."'";
    if(!$region_id=$GLOBALS['db']->getRow($sql)){
        api_err('0x003','state error');
    }
    $aAddr['province'] = $region_id['region_id'];

    $sql="SELECT region_id FROM ".$GLOBALS['xishan']->table('region')." WHERE region_type = '2' AND region_name = '".$city."'";
    if(!$region_id=$GLOBALS['db']->getRow($sql)){
        api_err('0x003','city error');
    }
    $aAddr['city'] = $region_id['region_id'];

    $sql="SELECT region_id FROM ".$GLOBALS['xishan']->table('region')." WHERE region_type = '3' AND region_name = '".$district."'";
    if(!$region_id=$GLOBALS['db']->getRow($sql)){
...
function update_order_status(){
    ...
    $type = $data['type'];
    $lastmodify = gmtime();
    $sql = "SELECT * FROM ".$GLOBALS['xishan']->table('order_info')." WHERE order_sn = ".$order_sn;
    $_order = $GLOBALS['db']->getRow($sql);
    if (!$_order) {
        api_err('','订单号“'.$order_sn.'”不存在');
    }

    $sql = " update ".$GLOBALS['xishan']->table('order_info')." ";
    $loginfo = array();
    switch ($type){
        ...
        case 'status' : //订单状态
            ...
            switch($status){
                ...
                case 'pending':
                    ...
                    break;
                case 'dead':
                    if($row = $GLOBALS['db']->getOne("select order_id from ".$GLOBALS['xishan']->table('order_info')." where order_sn = {$order_sn} and (pay_status = 0 or order_amount=0)")){
                        $loginfo['msg'] = $msg.'死单';
                        $loginfo['behavior'] = '死单';
                        $status = 2;
                    }else{
...
function search_goods_list()
{
    ...
    if (is_numeric($_POST['last_modify_st_time']) && is_numeric($_POST['last_modify_en_time']))
    {
        $sql = 'SELECT COUNT(*) AS count' .
            ' FROM ' . $GLOBALS['xishan']->table('goods') .
        ...
        /* 处理更新时间等于0的数据 */
        $sql = 'UPDATE ' . $GLOBALS['xishan']->table('goods') .
...
function search_goods_detail()
{
    ...
    if (!empty($_POST['goods_id']) && is_numeric($_POST['goods_id']))
    {
        $sql = 'SELECT g.goods_id, g.last_update AS last_modify, g.cat_id, c.cat_name AS category_name, g.brand_id, b.brand_name, g.shop_price AS price, g.goods_sn AS bn, g.goods_name AS name, g.is_on_sale AS marketable, g.goods_weight AS weight, g.goods_number AS store , g.give_integral AS score, g.add_time AS uptime, g.original_img AS image_default, g.goods_desc AS intro' .
            ' FROM ' . $GLOBALS['xishan']->table('category') . ' AS c, ' . $GLOBALS['xishan']->table('goods') . ' AS g LEFT JOIN ' . $GLOBALS['xishan']->table('brand') . ' AS b ON g.brand_id = b.brand_id'.
...
function search_site_info()
{
    ...
    $sql = 'SELECT code, value'.
        ' FROM ' . $GLOBALS['xishan']->table('shop_config') .
...
function create_goods_properties($goods_id)
{
    ...
    $sql = "SELECT attr_group ".
            "FROM " . $GLOBALS['xishan']->table('goods_type') . " AS gt, " . $GLOBALS['xishan']->table('goods') . " AS g ".
    ...
    */
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
        ...
        'FROM ' . $GLOBALS['xishan']->table('goods_attr') . ' AS g ' .
        'LEFT JOIN ' . $GLOBALS['xishan']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
...
function get_certinfo(){
    ...
    //验证是否有eid的账号认证过
    $row = $GLOBALS['db']->getRow("select user_id from ".$GLOBALS['xishan']->table('admin_user')." where passport_uid = '".trim($data['eid'])."'");
    if($row){
        //获取证书
        ...
        if($node_id and $cert_id and $token){
            ...
            if($res['res']=='succ'){
                ...
                $sql = "insert into ".$GLOBALS['xishan']->table('shop_config')." set parent_id=2,code='bind_taodali',type='hidden',value='".serialize($data)."'";
...
function fy_logistics_offline_send(){
    ...
    if (!empty($invoice_no)){
        ...
        $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
            ...
            "FROM " . $xishan->table('order_info') . " AS o " .
            "LEFT JOIN " . $xishan->table('region') . " AS c ON o.country = c.region_id " .
            "LEFT JOIN " . $xishan->table('region') . " AS p ON o.province = p.region_id " .
            "LEFT JOIN " . $xishan->table('region') . " AS t ON o.city = t.region_id " .
            "LEFT JOIN " . $xishan->table('region') . " AS d ON o.district = d.region_id " .
        ...
        $sql ="select add_time from ". $xishan->table('order_info') ." WHERE order_sn = '" . $delivery['order_sn'] . "'";
        ...
        /* 发货单入库 */
        $query = $db->autoExecute($xishan->table('delivery_order'), $_delivery, 'INSERT', '', 'SILENT');
            ...
            if (!empty($goods_list)){
                foreach ($goods_list as $value){
                    ...
                    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card'){
                        ...
                        $query = $db->autoExecute($xishan->table('delivery_goods'), $delivery_goods, 'INSERT', '', 'SILENT');
                        $sql = "UPDATE ".$xishan->table('order_goods'). " SET send_number = " . $value['goods_number'] . " WHERE order_id = '" . $value['order_id'] . "' AND goods_id = '" . $value['goods_id'] . "' ";
                        $db->query($sql, 'SILENT');
                        ...
                        foreach ($value['package_goods_list'] as $pg_key => $pg_value){
                            ...
                            $query = $db->autoExecute($xishan->table('delivery_goods'), $delivery_pg_goods, 'INSERT', '', 'SILENT');
                            $sql = "UPDATE ".$xishan->table('order_goods'). " SET send_number = " . $value['goods_number'] . " WHERE order_id = '" . $value['order_id'] . "' AND goods_id = '" . $pg_value['goods_id'] . "'";
                            $db->query($sql, 'SILENT');
                        }
                    }
                }
        ...
        FROM " . $xishan->table('delivery_goods') . " AS DG, " . $xishan->table('goods') . " AS G, " . $xishan->table('products') . " AS P
        ...
        FROM " . $xishan->table('delivery_goods') . " AS DG, " . $xishan->table('goods') . " AS G
        WHERE DG.goods_id = G.goods_id
        AND DG.delivery_id = '$delivery_id'
        ...
        if ($GLOBALS['_CFG']['use_storage'] == '1' && $GLOBALS['_CFG']['stock_dec_time'] == SDT_SHIP){
            foreach ($delivery_stock_result as $value){
                ...
                if ($value['is_real'] != 0){
                    //（货品）
                    if (!empty($value['product_id'])){
                        $minus_stock_sql = "UPDATE " . $GLOBALS['xishan']->table('products') . "
                                        SET product_number = product_number - " . $value['sums'] . "
                                        WHERE product_id = " . $value['product_id'];
                        $db->query($minus_stock_sql, 'SILENT');
                    }
                    $minus_stock_sql = "UPDATE " . $GLOBALS['xishan']->table('goods') . "
        ...
        $query = $db->autoExecute($xishan->table('delivery_order'), $_delivery, 'UPDATE', "delivery_id = $delivery_id", 'SILENT');
        if (!$query){
        ...
        if ($order_finish){
            ...
            if ($cfg == '1'){
                ...
                $GLOBALS['smarty']->assign('confirm_url', $xishan->url() . 'receive.php?id=' . $order['order_id'] . '&con=' . rawurlencode($order['consignee']));
                $GLOBALS['smarty']->assign('send_msg_url',$xishan->url() . 'user.php?act=message_list&order_id=' . $order['order_id']);
...
function verify_order_valid($order_sn,&$order,$colums='*',&$msg){
    $sql = "select ".$colums." from ".$GLOBALS['xishan']->table('order_info')." where order_sn='".$order_sn."'";
    $_order = $GLOBALS['db']->getRow($sql);
    if(!$_order){
        $msg = '订单号“'.$order_sn.'”不存在';
        return false;
    }else{
        $sql = "select * from ".$GLOBALS['xishan']->table('order_goods')." where order_id=".$_order['order_id'];
...
function get_order_goods($order){
    ...
    $sql = "SELECT o.*, g.suppliers_id AS suppliers_id,IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn " .
        "FROM " . $GLOBALS['xishan']->table('order_goods') . " AS o ".
        "LEFT JOIN " . $GLOBALS['xishan']->table('products') . " AS p ON o.product_id = p.product_id " .
        "LEFT JOIN " . $GLOBALS['xishan']->table('goods') . " AS g ON o.goods_id = g.goods_id " .
        "LEFT JOIN " . $GLOBALS['xishan']->table('brand') . " AS b ON g.brand_id = b.brand_id " .
        "WHERE o.order_id = '$order[order_id]' ";
...
function get_order_finish($order_id){
    ...
    $sql = 'SELECT COUNT(rec_id)
            FROM ' . $GLOBALS['xishan']->table('order_goods') . '
...
function delivery_order_info($delivery_id, $delivery_sn = '')
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['xishan']->table('delivery_order');
    if ($delivery_id > 0)
    {
...
function get_all_delivery_finish($order_id){
    ...
    else
    {
        ...
        $sql = "SELECT COUNT(delivery_id)
                FROM " . $GLOBALS['xishan']->table('delivery_order') . "
                WHERE order_id = '$order_id'
                AND status = 2 ";
        $sum = $GLOBALS['db']->getOne($sql);
        // 全部发货
...
function update_store(){
    ...
    foreach ($result as $val) {
        ...
        if( checkStore($val['bn'],$memo['last_modified'],$val) ){
            if ($goods_id = $GLOBALS['db']->getOne("SELECT goods_id FROM ".$GLOBALS['xishan']->table('products')." WHERE product_sn = '".$val['bn']."'") ) {
                // 多规格商品
                $sql = "update " . $GLOBALS['xishan']->table('products') . "  set product_number={$val['store']} where product_sn = '".$val['bn']."' ";
                if ( $GLOBALS['db']->query($sql) ){
                    $sql = "update ".$GLOBALS['xishan']->table('goods') ." set goods_number = (SELECT sum(product_number) FROM ".$GLOBALS['xishan']->table('products')." WHERE goods_id = {$goods_id}) WHERE goods_id='{$goods_id}'";
                    if ( $GLOBALS['db']->query($sql) ){
                ...
                if ($goods_id = $GLOBALS['db']->getOne("SELECT goods_id FROM ".$GLOBALS['xishan']->table('goods')." WHERE goods_sn = '".$val['bn']."'") ) {
                    // 单规格商品
                    $sql_goods = "update ".$GLOBALS['xishan']->table('goods')." set goods_number = {$val['store']} WHERE goods_sn='".$val['bn']."'";
...
function checkStore($bn,$time,$val){
    $time = time();
    $sql = "select type_id,time FROM  " . $GLOBALS['xishan']->table('coincidence') . " WHERE type_id='".$bn."' and type='UPDATE_STORE' ";
    if($row = $GLOBALS['db']->getRow($sql)){
        if( $row['time'] < $time ){
            $sql = "update " . $GLOBALS['xishan']->table('coincidence') . " set time={$time} WHERE type_id='".$bn."' and type='UPDATE_STORE'  ";
            $res = $GLOBALS['db']->query($sql);
            return true;
        }
    }else{
        $sql = "insert into " . $GLOBALS['xishan']->table('coincidence') . " set type_id='".$bn."',type='UPDATE_STORE',time={$time} ";
        $res = $GLOBALS['db']->query($sql);
        return true;
    }
    return false;
...
function get_payment_conf(){
    ...
    $return_data = array();
    $sql = "select pay_id as payment_id, pay_name as payment_name,IF(is_online=1, 'online', 'offline') as payout_type , pay_code as payment_bn  from  ". $GLOBALS['xishan']->table('payment') . "  where enabled = 1";
...
function ome_create_payments(){
    ...
    $sql = "select order_id,order_sn,order_amount,money_paid,shipping_status,user_id from " . $GLOBALS['xishan']->table('order_info') . " where  order_sn = '".$data['order_id']."' and pay_status = ".PS_UNPAYED." ";
    ...
    $sql = "select pay_id,pay_name from " . $GLOBALS['xishan']->table('payment') . " where pay_code = '".$data['payment']."'  ";
...
function ome_create_reimburse(){
    ...
    // $sql = "select * from " . $GLOBALS['xishan']->table('order_info') . " where  order_sn = '".$data['order_id']."' and order_status != ".OS_RETURNED." and pay_status = ".PS_PAYED." and shipping_status = ".SS_UNSHIPPED." ";
    $sql = "select *,money_paid+surplus as payed_all from " . $GLOBALS['xishan']->table('order_info') . " where  order_sn = '".$data['order_id']."' and money_paid+surplus>0";
    $order = $GLOBALS['db']->getRow($sql);
    if(!$order){
    ...
    if ($data['cur_money'] >= $order['payed_all']) {
        ...
        //     $GLOBALS['db']->query("UPDATE ". $GLOBALS['xishan']->table('order_info') . " SET `order_amount` = '0' WHERE `order_id` =". $order['order_id']);
        // }
        /* 退货积分 */
...
function ome_create_delivery(){
    ...
    $sql = "select * from " . $GLOBALS['xishan']->table('order_info') . " where  order_sn = '".$data['order_id']."' ";
    $order = $GLOBALS['db']->getRow($sql);
    if(!$order){
        api_err('0x003', 'order_id no exist');
    }
    ...
    // 匹配快递名称
    if( $data['logi_name'] ){
        $sql = "select shipping_id from ".$GLOBALS['xishan']->table('shipping')." where shipping_name = '".$data['logi_name']."' ";
    ...
    if( $data['logi_id'] and !$match_logi ){
        $sql = "select shipping_id from ".$GLOBALS['xishan']->table('shipping')." where shipping_code = '".strtolower($data['logi_id'])."' ";
        if($shipping_id = $GLOBALS['db']->getOne($sql)){
            $match_logi = true;
            $shipping['shipping_id'] = $shipping_id;
            $shipping['shipping_name'] = $data['logi_name'];
    ...
    if( $data['type'] == 'delivery' ){// todo 处理发货
        ...
        // 判断发货单是否存在
        $sql = "select delivery_id from ".$GLOBALS['xishan']->table('delivery_order')." where delivery_sn = ".$data['delivery_id'];
        ...
        $unship_num = 0;
        /* 发货单入库 */
        $query = $GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('delivery_order'), $delivery, 'INSERT', '', 'SILENT');
        $delivery_id = $GLOBALS['db']->insert_id();
        if ($delivery_id)
        {
            $sql = "select * from ".$GLOBALS['xishan']->table('order_goods')." where order_id = ".$order['order_id'];
            ...
            if($order_goods){
                ...
                $sql = "select g.*,(g.goods_number - g.send_number) as need_send_number,ifnull(p.`product_sn`,g.`goods_sn`) as product_sn from ".$GLOBALS['xishan']->table('order_goods')." as g left join ".$GLOBALS['xishan']->table('products')." as p on p.goods_id = g.goods_id  where g.order_id = ".$order['order_id'];
                $order_goods = $GLOBALS['db']->getAll($sql);
                ...
                foreach($order_goods as $value){
                    ...
                    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card')
                    {
                        ...
                        $query = $GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('delivery_goods'), $delivery_goods, 'INSERT', '', 'SILENT');
                    }
                    // 商品（超值礼包）
                    elseif ($value['extension_code'] == 'package_buy')
                    {
                        foreach ($value['package_goods_list'] as $pg_key => $pg_value)
                        {
                            ...
                            $query = $GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('delivery_goods'), $delivery_pg_goods, 'INSERT', '', 'SILENT');
                        }
                    }
                    //更新订单明细中的发货值
                    $sql = "update ".$GLOBALS['xishan']->table('order_goods')." set send_number = send_number + ".$send_number."  where order_id = ".$order['order_id']." and rec_id = ".$value['rec_id'];
                    $GLOBALS['db']->query($sql);
                }
            }
        ...
        // 判断退货单是否存在
        $sql = "select back_id from ".$GLOBALS['xishan']->table('back_order')." where delivery_sn = ".$data['delivery_id'];
        if($GLOBALS['db']->getOne($sql)){
        ...
        /* 退货单入库 */
        $query = $GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('back_order'), $back_order, 'INSERT', '', 'SILENT');
        ...
        $sql = "select g.* , ifnull(p.`product_sn`,g.`goods_sn`) as product_sn from ".$GLOBALS['xishan']->table('order_goods')." as g left join ".$GLOBALS['xishan']->table('products')." as p on p.goods_id = g.goods_id  where g.order_id = ".$order['order_id'] ." and g.send_number > 0 ";
        $order_goods = $GLOBALS['db']->getAll($sql);
            ...
            foreach ($order_goods as $value)
            {
                ...
                if ( $delivery_bns[$value['product_sn']] and $delivery_bns[$value['product_sn']] <= $value['send_number'] ) {
                    ...
                    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card')
                    {
                        ...
                        $query = $GLOBALS['db']->autoExecute($GLOBALS['xishan']->table('back_goods'), $back_goods, 'INSERT', '', 'SILENT');

                        /* 将订单的商品发货数量更新 */
                        $sql = "UPDATE " . $GLOBALS['xishan']->table('order_goods') . "
        ...
        $sql = "select sum(send_number) from ".$GLOBALS['xishan']->table('order_goods')." where order_id = ".$order['order_id'];
        ...
        /* 修改订单的发货单状态为退货 */
        $sql_delivery = "UPDATE " . $GLOBALS['xishan']->table('delivery_order') . "
                         SET status = 0
                         WHERE status IN (1, 2)
        ...
        if ($order['user_id'] > 0)
        {
            ...
            $sql = "SELECT  goods_number, send_number FROM". $GLOBALS['xishan']->table('order_goods') . "
...
function shopex_shop_login()
{
    ...
    $cert = new certificate();
    if ($db->getRow("SELECT user_id FROM " . $xishan->table('admin_user') . " WHERE passport_uid = '" . trim($_POST['passport_uid']) . "'")) {
        $license = $cert->get_shop_certificate();
        if ($license['passport_uid'] && $license['passport_uid'] == trim($_POST['passport_uid'])) {
            ...
            $re_arr['site_address'] = $xishan->url();
            ...
            $re_arr['goods_url'] = $xishan->url() . 'goods.php?id=0'; //站点网址
...
function shopex_goods_cat_list()
{
    ...
    $sql = "SELECT cat_id,cat_name,cat_desc,parent_id,sort_order FROM " . $xishan->table('category');
...
function shopex_brand_list()
{
    ...
    $sql = "SELECT brand_name,site_url,brand_desc,sort_order FROM " . $xishan->table('brand');
    if ($res = $db->getAll($sql)) {
        $re_arr = array();
        foreach ($res as $k => $v) {
...
function shopex_type_list()
{
    ...
    $sql = "SELECT gt.cat_id AS type_id,gt.cat_name AS type_name,a.attr_id,a.attr_name,a.attr_values,a.sort_order " .
        "FROM " . $xishan->table('attribute') . " AS a LEFT JOIN " . $xishan->table('goods_type') . " AS gt ON a.cat_id = gt.cat_id " .
...
function shopex_goods_add()
{
    ...
    $re_arr['goods_url'] = $GLOBALS['xishan']->url() . "/goods.php?id={$goods['goods_id']}";
...
function goodsType($goods_data, &$type_id)
{
    ...
    $sql = "SELECT cat_id FROM " . $xishan->table('goods_type') . " WHERE cat_name = '{$type_name}'";
    if (($type_id = $db->getOne($sql)) == false) {
        $sql = "INSERT INTO " . $xishan->table('goods_type') . " (`cat_name`,`enabled`) VALUE ('{$type_name}',1)";
...
function goods($goods_data, $cat_id, $type_id, &$goods)
{
    ...
    if ($brand) {
        $sql = "SELECT brand_id FROM " . $xishan->table('brand') . " WHERE brand_name = '{$brand}'";
    ...
    $sql = "SELECT * FROM " . $xishan->table('goods') . " WHERE goods_sn = '" . $goods_arr['goods_sn'] . "'";
    if ($res = $db->getOne($sql)) {
        unset($goods_arr['add_time']);
        $sets = '';
        foreach ($goods_arr as $k => $v) $sets .= "`{$k}`" . " = '" . $v . "',";
        $sets = trim($sets, ',');
        $sql = "UPDATE " . $xishan->table('goods') . " SET " . $sets . " WHERE goods_sn = '{$goods_arr['goods_sn']}'";
    } else {
        $sql = "INSERT INTO " . $xishan->table('goods') . " (`" . implode('`,`', array_keys($goods_arr)) . "`) " . "VALUES ('" . implode("','", array_values($goods_arr)) . "')";
    }
    ...
    //返回商品数据
    $goods = $db->getRow("SELECT * FROM " . $xishan->table('goods') . " WHERE goods_sn = '" . $goods_arr['goods_sn'] . "'");
}
...
function attribute($goods_data, $goods, $type_id)
{
    ...
    //处理已存在的属性
    $sql = "SELECT * FROM " . $xishan->table('attribute') . " WHERE cat_id = {$type_id} AND attr_name IN ('" . implode("','", array_keys($attr_arr)) . "')";
    if ($res = $db->getAll($sql)) {
        //整理属性数据
        $res_arr = array();
        foreach ($res as $attr) {
        ...
        foreach ($attr_arr as $name => $value) {
            ...
            if (isset($res_arr[$name]) == true) {
                ...
                if ($diff = array_diff($value, $res_arr[$name])) {
                    ...
                    $sql = "UPDATE " . $xishan->table('attribute') . " SET `attr_values` = \"" . implode("\n", $attr_value) . "\" WHERE cat_id = {$type_id} AND attr_name = '{$name}'";
    ...
    //新增属性
    if ($attr_arr) {
        $sql = "INSERT INTO " . $xishan->table('attribute') . " (`cat_id`,`attr_name`,`attr_values`,`attr_input_type`,`attr_type`) VALUES ";
    ...
    $attribute = $db->getAll("SELECT * FROM " . $xishan->table('attribute') . " WHERE cat_id = {$type_id}");
    $attribute_arr = array();
    ...
    //处理已存在的商品属性
    if ($res = $db->getAll("SELECT * FROM " . $xishan->table('goods_attr') . " AS g LEFT JOIN " . $xishan->table('attribute') . " AS a ON g.attr_id = a.attr_id WHERE g.goods_id = {$goods['goods_id']}")) {
        //整理商品属
        $res_arr = array();
        foreach ($res as $v) {
            if (!empty($v['attr_values'])) {
            ...
            if ($goods_attr_arr[$name]) {
                foreach ($goods_attr_arr[$name] as $k => $v) {
                    if (in_array($v, reset($values))) {
                        if ($type == 0 && $v != $value) {
                            $sql = "UPDATE " . $xishan->table('goods_attr') . " SET attr_value = '{$v}' WHERE goods_id = {$goods['goods_id']} AND attr_id = " . key($values) . " AND goods_attr_id = {$goods_attr_id}";
    ...
    //新增商品属性
    if ($goods_attr_arr) {
        $sql = "INSERT INTO " . $xishan->table('goods_attr') . " (`goods_id`,`attr_id`,`attr_value`,`attr_price`) VALUES ";
    ...
    $goods_attr = $db->getAll("SELECT * FROM " . $xishan->table('goods_attr') . " WHERE goods_id = {$goods['goods_id']}");
    ...
    $db->query("UPDATE " . $xishan->table('attribute') . " SET attr_type = 1 WHERE cat_id = {$type_id} AND attr_name IN ('" . implode("','", $sku_keys) . "')");

    if ($res = $db->getAll("SELECT product_sn FROM " . $xishan->table('products') . " WHERE product_sn IN ('" . implode("','", $sku_ids) . "')")) {
        ...
        foreach ($sku_arr as $bn => $sku) {
            if (in_array($bn, array_values($res_arr))) {
                ...
                $sql = "UPDATE " . $xishan->table('products') . " SET goods_attr = '{$goods_attr}', product_number = {$quantities} WHERE product_sn = '{$bn}'";
    ...
    if ($sku_arr) {
        $sql = "INSERT INTO " . $xishan->table('products') . " (`goods_id`,`goods_attr`,`product_sn`,`product_number`) VALUES ";
        foreach ($sku_arr as $bn => $sku) {
            $goods_attr = '';
            foreach ($sku as $k => $v) {
                $goods_attr .= $v['goods_attr_id'] . '|'; //规格
...
function gallery($goods_data, $goods)
{
    ...
    //删除不存在的图片
    $sql = "DELETE FROM " . $xishan->table('goods_gallery') . " WHERE `goods_id` = {$goods['goods_id']} AND `img_original` NOT IN ('" . implode("','", $images) . "')";
    $db->query($sql);

    //查找已有图片
    $sql = "SELECT img_original FROM " . $xishan->table('goods_gallery') . " WHERE `goods_id` = {$goods['goods_id']} AND `img_original` IN ('" . implode("','", $images) . "')";
    $tmp_img_arr = array();
    if ($res = $db->getAll($sql)) {
        foreach ($res as $v) {
    ...
    if ($images = array_diff($images, $tmp_img_arr)) {
        ...
        foreach ($images as $img) {
            ...
            if ($img == $defaultimage) {
                ...
                $sql = "UPDATE " . $xishan->table('goods') . " SET goods_thumb = '{$thumb_default_image}', goods_img = '{$thumb_default_image}', original_img = '{$img}' WHERE `goods_id` = {$goods['goods_id']}";
                $db->query($sql);
            }
        }
    }
...
function shopex_goods_search()
{
    ...
    $re_arr = array();
    $re_arr['item_total'] = $db->getOne("SELECT count(*) FROM " . $xishan->table('goods'));

    $cat_list = cat_list(0, 0, false);
    $brand_list = get_brand_list();
    if ($res = $db->getAll("SELECT * FROM " . $xishan->table('goods_type'))) {
        foreach ($res as $v) {
            $type_list[$v['cat_id']] = $v['cat_name'];
        }
    }

    $sql = "SELECT * FROM " . $xishan->table('goods') . " LIMIT " . ($page_no - 1) * $page_size . ",{$page_size}";
    if ($res = $db->getAll($sql)) {
        foreach ($res as $k => $goods) {
            ...
            //默认图片
            $default_image_path = $goods['goods_img'] ? $xishan->url() . $goods['goods_img'] : '';
            ...
            $goods['goods_desc'] = htmlspecialchars_decode(preg_replace("/src=[\'|\"][^http](.*?(?:[\.gif|\.jpg|\.png]))[\'|\"]/i", "src=\"" . $xishan->url() . "$1\"", $goods['goods_desc']));
            $re_arr['goods'][$gid]['intro'] = $goods['goods_desc']; //商品详细信息

            //属性 & 货品
            $products_props = array();
            $prop_values = $products = $spec_values = '';
            $sql = "SELECT * FROM " . $xishan->table('goods_attr') . " AS g LEFT JOIN " . $xishan->table('attribute') . " AS a "
            ...
            $sql = "SELECT * FROM " . $xishan->table('products') . " WHERE goods_id = {$goods['goods_id']}";
            if ($res = $db->getAll($sql)) {
                foreach ($res as $pk => $v) {
            ...
            //商品图片
            $goods_images = '';
            $sql = "SELECT * FROM " . $xishan->table('goods_gallery') . " WHERE goods_id = {$goods['goods_id']}";
            if ($res = $db->getALL($sql)) {
                foreach ($res as $k => $v) {
                    preg_match('/^http:\/\//', $v['img_url'], $matches);
                    if (empty($matches)) {
                        $goods_images[$k]['source'] = $xishan->url() . '/' . $v['img_url'];
                    } else {
            ...
            $re_arr['goods'][$gid]['goods_url'] = $xishan->url() . "goods.php?id={$goods['goods_id']}"; //商品详前台URL
        }
        api_response('true', '', $re_arr, RETURN_TYPE);
    }
    api_response('true', 'No Data', '', RETURN_TYPE);
