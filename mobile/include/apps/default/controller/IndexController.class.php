...
class IndexController extends CommonController {
    ...
    public function index() {
        ...
        // $goods_all= model('Common')->get_goods_all();
        ...
        $this->assign('brand_list', model('Brand')->get_brands($app = 'brand', C('page_size'), 1));
            //获取所有商品
            $sql1 = "SELECT * from ecs_goods where user_id='1'";
            //获取热卖商品
            $sql2 = "SELECT * from ecs_goods where user_id='1' and is_hot='1'";
            //获取精品商品
            $sql3 = "SELECT * from ecs_goods where user_id='1' and is_best='1'";

            $goods_all = $this->model->query($sql1);
            $this->assign('goods_all', $goods_all);
            $goods_all2 = $this->model->query($sql2);
            $this->assign('goods_all2', $goods_all2);
            $goods_all3 = $this->model->query($sql3);
            $this->assign('goods_all3', $goods_all3);
    ...
    public function dingwei() {
        ...
        header("Content-type: text/html; charset=utf-8");
         $ip =  model('Index')->get_client_ip();

        $city = model('Index')->convertip($ip); //通过QQ纯真IP库获取相关信息
         $this->assign('ip', $ip);
