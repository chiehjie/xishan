...
class UserController extends CommonController {
    ...
    public function index() {
        ...
        $sql="select user_money from ecs_users where user_id='".$_SESSION['user_id']."'";
        $user_money=$this->model->query($sql);
        ...
        $this->display('my_user.dwt');
    }
// public function user_money(){
//     $sql="select user_money from ecs_users where user_id='".$_SESSION['user_id'];
//     $res=$this->model->query($sql);
//     $this->assign('user_money',$res );
...
// }
    ...
    public function account_detail() {
        ...
        $this->display('my_recharge.dwt');
    }
     public function my_chongzhi(){


        $this->display('my_chongzhi.dwt');
    }
     ...
     * 会员转账-转账给朋友
     */
    public function account_transfer_myfirend(){
        $this->display('user_account_transfer_myfirend.dwt');
    }

    /**
     * 会员转账-转账给账户
     */
    public function account_transfer_account(){
		//var_dump($_SESSION['user_id']);exit;
        $this->display('user_account_transfer_account.dwt');
    }

    /**
     * 会员转账-账户名称
     */
    public function account_transfer_name(){
		     
        $names = $_POST['names']; 		
        $_SESSION['names'] = $names;
		if($names==$_SESSION['user_name']){
			echo "<script>alert('不能向自己账户转账');history.back();</script>";
			//$this->display('user_account_transfer_account.dwt');
		}else{
			$sql = "SELECT * from ecs_users where user_name='".$names."'";
			$check = $this->model->query($sql); 
			//电话号码
			$mobile_phone = $check['0']['mobile_phone'];
			//电话号码不全显示
			$check['0']['mobile_phone']=substr_replace($mobile_phone,'****',3,4);
			$this->assign('check', $check);
			//var_dump($check['0']['mobile_phone']);exit;   

			if (!$check['0']['user_name']) {
				echo "<script>alert('账户不存在');history.back();</script>";
				//$this->display('user_account_transfer_account.dwt');
			}else
			{
				$this->display('user_transferamount.dwt');
			}
		}
     ...
     * 会员转账-转账金额
     */
    public function account_result(){
        if (IS_POST) {
            //转账者需要转的money
      ...
      // var_dump($_SESSION['user_id']);exit;
        ...
        $user_money = $this->model->query($sql);
        //转账者转账前的money
        $quser_money = (float)$user_money['0']['user_money'];
        //转账者转账后的money
        $huser_money = $quser_money - $money;
        //转账者转账时间
        $time = date("Y-m-d H:i:s");

        $sql2 = "SELECT user_money from ecs_users where user_name='".$_SESSION['names']."'";
        $u_money = $this->model->query($sql2);
        //接受者接受前的money
        $qquser_money = (float)$u_money['0']['user_money'];
        //接受者接受后的money
        $hhuser_money = $qquser_money + $money;
        
        //确认转账后,跳转到密码页
        //$this->display('user_account_result.dwt');
        
        //判断余额
        if ($quser_money < $money) {
            echo "<script>alert('账户余额不足');history.back();</script>";
            //$this->display('user_transferamount.dwt');
        }else{
            //转账者账户更新
            $sql = "UPDATE ecs_users set user_money='".$huser_money."' where user_id='".$id."'";
            $check1 = $this->model->query($sql);
            //转账后账户余额
            $sql = "SELECT user_money from ecs_users where user_id='".$id."'";
            $u_money = $this->model->query($sql);
            $this->assign('u_money',$u_money);
            //var_dump($u_money);exit;
            //接受者者账户更新
            $sql2 = "UPDATE ecs_users set user_money='".$hhuser_money."' where user_name='".$_SESSION['names']."'";
            $check2 = $this->model->query($sql2);
            $this->display('user_account_result.dwt');
        }
     }
    ...
    public function account_recharge(){
		...
		$_SESSION['user_id']=$this->user_id ;
        $sql = "SELECT user_money from ecs_users where user_id='".$_SESSION['user_id']."'";
        $money = $this->model->query($sql);
        //var_dump($money);exit;
        $this->assign('money',$money);
        $this->display('my_user_recharge.dwt');
    }

    /**
     * 明细
     */
    public function account_transferdetail(){
        $this->display('user_account_transferdetail.dwt');
    }

    /**
     * 转账记录
     */
    public function account_transferecord(){
        $this->display('user_account_transferecord.dwt');
    }
    ...
    public function recharge_method(){
        $this->display('user_recharge_method.dwt');
    }
     ...
     * 转账-转账给银行
     */
    public function account_transfer_bank(){
		$_SESSION['user_id']=$this->user_id;
        $this->display('user_account_transbanks.dwt');
    }
	public function account_transfer_banktwo(){
		if(IS_POST)
		{
			session_start();
			$_SESSION['user_id']=$this->user_id;
			$name = $_POST['username'];
			$bank_card = $_POST['card'];
			$money = (float)$_POST['money'];
			$_SESSION['$name'] = $name;
			$_SESSION['$bank_card'] = $bank_card;
			...
			$quser_money = (float)$user_money['0']['user_money'];
			//转账者转账后的money
			$huser_money = $quser_money - $money - 2 - ($money * 0.007);
			//转账者转账时间
			$time = date("Y-m-d H:i:s");
			
			$data = array(
			array('name'=>$_SESSION['$name'],
					'card'=>$_SESSION['$bank_card'],
			...
			//配置您申请的appkey
			$appkey = "a12847bbb96eb38370ef3293d6854a0f";
			//************1.银行卡实名认证查询************
			...
			$content = yinhangka($url,$paramstring);
			...
			if($result)
			{
				 if($result['error_code']=='0')
				 {
					if($quser_money < $money)
					{
						...
						//$this->display('user_account_transbanks_two.dwt');
						echo "<script>alert('账户余额不足');history.back();</script>";  
					}
            ...
            {
				echo "<script>alert('银行卡卡号错误');history.back();</script>";
			}
		}
    }
  
    public function yhkcx(){
...
if($result){
    ...
    echo "请求失败";
		...
		* 转账-转账给银行-转账结果
		*/
	public function account_transfer_bankthree(){
		session_start();
		$_SESSION['user_id']=$this->user_id;
		$_SESSION['$time'] = date("Y-m-d H:i:s",strtotime("+1 day"));
		$data = array(
			array('name'=>$_SESSION['$name'],
					'card'=>$_SESSION['$bank_card'],
					'money'=>$_SESSION['$money'],
		...
		//var_dump($data);
		$this->assign('data',$data);
        $this->display('user_account_transbanks_three.dwt');
    }
    ...
    public function share() {
        ...
        // $shopurl = "http://www.cd-seasons.com/mobile/index.php?m=default&c=brand" . $this->user_id;
          $shopurl="http://www.cd-seasons.com/mobile/index.php?m=default&c=user&a=login";
    ...
    public function create_qrcode(){
       $value ="http://www.cd-seasons.com/mobile/index.php?m=default&c=user&a=login";
        if($value){
            // 二维码
            // 纠错级别：L、M、Q、H
    ...
    public function login() {
        // 登录处理
        $this->display('my_user_login.dwt');
    }
    public function dologin(){
        if (IS_POST) {
            ...
            if (self::$user->login($username, $password, isset($_POST['remember']))) {
                ...
                $jump_url = empty($this->back_act) ? url('index') : $this->back_act;
                // var_dump($jump_url);exit;
                $this->redirect($jump_url);
            } else {
                $_SESSION['login_fail']++;
                // var_dump($_SESSION['login_fail']);exit;
                // var_dump($username);exit;
                // var_dump($password);exit;
    ...
    /*修改密码一*/
    public function my_getpassword_first(){
        // var_dump(123);exit;
         $this->display('my_getpassword_first.dwt');
    }
    /*修改密码二*/
    public function my_getpassword_two(){
         $this->display('my_getpassword_two.dwt');
    }
    /*修改密码短信验证*/
    public function my_getpassword_sms(){
        session_start();
        $mobile = $_POST['mobile'];
        ...
        // var_dump($smsConf);exit;
        $_SESSION['code'] = $code;
        $_SESSION['mobile'] = $mobile;
        // var_dump($_SESSION['mobile']);exit;
        $content = juhecurl($sendUrl,$smsConf,1);
    }


    /*修改密码三邮箱密码找回*/
    public function my_getpassword_three(){
        // var_dump(123);exit;
         $this->display('my_getpassword_three.dwt');
    }
    /*邮箱接口*/
    public function my_getpassword_email_sms() {
    ...
    /*邮箱密码找回*/
    public function my_getpassword_email() {
        $email = $_POST['email'];
        $code = $_SESSION['code'];
        $_SESSION['email'] = $email;
        // var_dump($code);exit;
        $sql = "select email from ". $this->model->pre ."users where email='" . $email. "'";
        $res = $this->model->query($sql);
        // var_dump($res);exit;

          if($res[0]['email'] == $email && $_SESSION['code'] == $_POST['code'] && !empty($email) &&!empty($_POST['code'])){
                $where['email'] = $email;
                ...
                $this->display('my_getpassword_last_email.dwt');
                exit();
        }else{
            // $this->display('my_getpassword_three.dwt');
            show_message(L('edit_password_email'), L('back_page_up'), '', 'info');
        }
    ...
    /*修改密码成功*/
     public function my_getpassword_last_email() {
        if (IS_POST) {
            ...
            $confirm_password = isset($_POST['confirm_password']) ? in($_POST['confirm_password']) : '';
            $user_id = isset($_POST['uid']) ? intval($_POST['uid']) : $this->user_id;
            $code = $_SESSION['code']; // 邮件code
     ...
     * 手机找回密码
     */
    public function my_getpassword_last() {
            // 手机找回密码处理
        $mobile = $_SESSION['mobile'];
        $code = $_SESSION['code'];

        // var_dump($code);exit;
        if($_SESSION['mobile'] == $_POST['mobile'] && $_SESSION['code'] == $_POST['code']){
                ...
                $this->display('my_getpassword_last.dwt');
                exit();
        }
            show_message(L('edit_password_xinxi'), L('back_page_up'), '', 'info');
    }

    /*修改密码成功*/
    public function my_getpassword_ok(){
        // var_dump(123);exit;
        // $mobile = $_SESSION['mobile'];
        // $password = $_POST['password'];
        // // var_dump($_SESSION['passwd_answer']);exit;
        // $password1 = md5($password);
        // // var_dump($password1);exit;
        // $sql = "select password from ". $this->model->pre ."users where mobile_phone=$mobile";
        // $res = $this->model->query($sql);
        // // var_dump($res);exit;
        // if ($password1 != $res[0]['password'])
        // {
        //     $sql = 'UPDATE ' . M()->pre . 'users' . " SET `password`='$password1' WHERE `mobile_phone`='" . $mobile . "'";
        //     // var_dump($sql);exit;
        //     M()->query($sql);
        //     $this->display('my_getpassword_ok.dwt');
        // }else{
        //     $this->display('my_getpassword_last.dwt'); 
        // }      
    }
    ...
    /*密保修改密码四*/
    public function my_getpassword_four(){
        $this->display('my_getpassword_four.dwt');
    }

    /*修改密码四*/
    public function my_getpassword_question(){
        session_start();
        ...
        if($res[0]['user_name']){
            ...
            // show_message(L('edit_password_xinxi'), L('back_retry_answer'), '', 'info');
            $this->display('my_getpassword_four.dwt');
        }           
    }
    ...
    /*密保找回密码*/
    public function my_getpassword_doemail(){
        session_start();
        $user_name = $_SESSION['username'];
        if (IS_POST) {
            $passwd_answer = isset($_POST['passwd_answer']) ? in($_POST['passwd_answer']) : '';
            // var_dump($passwd_answer);exit;
            $sel_question = $_POST['sel_question'];
            // var_dump($sel_question);exit;
            // 取出会员密码问题和答案
            ...
            $user_question_arr = $this->model->table('users')
                    ...
                    ->where($where)
                    ->find();
                    // var_dump($user_question_arr['passwd_question']);exit;
            // 如果没有设置密码问题，给出错误提示
            if (empty($user_question_arr['passwd_answer']) || $sel_question != $user_question_arr['passwd_question']) {
                show_message(L('no_passwd_question'), L('back_retry_answer'), url('my_getpassword_question'), 'info');
            }

            // 问题答案验证
            if (empty($_POST['passwd_answer']) || in($_POST['passwd_answer']) != $user_question_arr['passwd_answer']) {
                show_message(L('wrong_passwd_answer'), L('back_retry_answer'), url('my_getpassword_question'), 'info');
            }
    ...
    public function my_xuanzhezhifufangshi(){
        // var_dump(123);exit;
       ...
       $this->assign('money', $_POST['money']);
        ...
        $this->display('my_xuanzhezhifufangshi.dwt');
    }
    ...
    // public function register() {
        ...
        // $this->display('my_user_regist.dwt');
    // }
    
     public function my_regist(){
        $this->display('my_user_regist.dwt');
     }
    /*注册三*/
    public function my_regist2() {
            /*
        ***聚合数据（JUHE.CN）短信API服务接口PHP请求示例源码
        ***DATE:2015-05-25*/
            // var_dump($_POST['mobile'])exit;
        session_start();
        $mobile = $_POST['mobile'];
        $code = rand(1000,9999);
        $sendUrl = 'http://v.juhe.cn/sms/send'; //短信接口的URL
        ...
        // $this->display('my_user_regist3.dwt');

    }

    public function my_regist3(){
        $mobile = $_POST['mobile'];
        if($_SESSION['mobile'] == $mobile && $_SESSION['code'] == $_POST['code']){
            $sql = "select mobile_phone from ". $this->model->pre ."users where mobile_phone=$mobile";
            $res = $this->model->query($sql);
            // var_dump($res);
            if(!$res[0]['mobile_phone']){
                $this->assign('extend_info_list', $extend_info_list);
                // // 密码提示问题
                $this->assign('password_question', L('passwd_questions'));
                $this->assign('title', L('register'));
               ...
               show_message(L('edit_password_mobile'), L('back_page_up'), '', 'info');
            }      
        }else{
            show_message(L('edit_password_mobile'), L('back_page_up'), '', 'info');
        }    
    ...
    /*注册四*/
    public function my_regist4() {
        // var_dump($_POST['passwd_answer']);exit;
        $sel_question = empty($_POST['sel_question']) ? '' : compile_str($_POST['sel_question']);
        $passwd_answer = isset($_POST['passwd_answer']) ? compile_str(trim($_POST['passwd_answer'])) : '';

        $_SESSION['sel_question'] = $sel_question;
        $_SESSION['passwd_answer'] = $passwd_answer;
        $this->display('my_user_regist4.dwt');

    }
    /*注册五*/
    public function my_regist5() {
        $password = $_POST['password'];
        // var_dump($_SESSION['passwd_answer']);exit;
        $password1 = md5($password);
        ...
        // var_dump($_SESSION['password']);exit;
        $this->model->query("INSERT INTO " . $this->model->pre . "users (`password` ,`user_name` ,`mobile_phone` ,`passwd_question` ,`passwd_answer`) VALUES ( '" . $password1 . "' , '" . $username . "' , '" . $_SESSION['mobile'] . "' , '" . $_SESSION['sel_question'] . "' , '" . $_SESSION['passwd_answer'] . "')");
        $this->display('my_user_regist5.dwt');
    }

    /*贷款*/
    public function my_loan(){

        $this->display('my_loan.dwt');

     }
     /*身份认证*/
    public function my_loan_first(){

    $this->display('my_loan_first.dwt');

    }
    /*贷款详情信息*/
    public function my_loan_two(){

    $this->display('my_loan_two.dwt');

    }
    /*贷款授权进度*/
    public function my_loan_last(){

    $this->display('my_loan_last.dwt');

    }
    
    /*开户*/
    public function my_OpenAccount(){

    $this->display('my_OpenAccount.dwt');

    }

    /*开户类型*/
    public function my_OpeningAccount(){

    $this->display('my_OpeningAccount.dwt');

    }

    /*开户信息*/
    public function my_openAccount3(){

    $this->display('my_openAccount3.dwt');

    }

    /*开户授权授权*/
    public function my_Authentication(){

    $this->display('my_Authentication.dwt');
    ...
    private function check_login() {
        ...
        $without = array(
            ...
            'add_collection',
            'third_login',
            'my_getpassword_first',
            'my_getpassword_two',
            'my_getpassword_three',
            'my_getpassword_four',
            'my_getpassword_last',
            'my_getpassword_ok',
            'my_getpassword_question',
            'my_xuanzhezhifufangshi',
            'my_getpassword_sms',
            'my_getpassword_email',
            'my_getpassword_email_sms',
            'my_getpassword_doemail',
            'my_getpassword_last_email',
			//'account_transfer_bankthree',
			//'account_recharge',
			//'account_recharge2'
            // 'account_result'
        );
      ...
      private  function yinhangka($url,$params=false,$ispost=0){
            $httpInfo = array();
            $ch = curl_init();
